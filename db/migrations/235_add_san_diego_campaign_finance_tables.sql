...
-- (F460 line 1 + line 4, never line 5 — it includes loans); cash_on_hand is
-- a signed balance (the GA/MA lesson of migrations 231/232);
-- direct_coverage_note persists the per-candidate coverage disclosure (e.g.
-- Martinez's committee opens 2025 with prior activity absent from the
-- export, and sub-$10k committees may file on paper — SDMC §27.29xx);
-- outside groups carry expenditure_count and include spender_name in the
...
CREATE TABLE public.sdcity_candidate_finance_links (
  ...
  -- FPPC id not yet assigned — San Diego workbooks also carry blank
  -- Filer_ID cells the shared parser normalizes to "Pending"). The resolver
  -- never links Pending committees; the constraint makes that
  -- unrepresentable here too — case-insensitively, so an upstream re-casing
  -- ("PENDING") surfaces as a loud write failure instead of a placeholder
  -- stored as a durable committee identity.
...
CREATE UNIQUE INDEX sdcity_candidate_finance_links_active_candidate_election_idx
  ON public.sdcity_candidate_finance_links (candidate_id, election_id) WHERE link_status = 'active';
CREATE INDEX sdcity_candidate_finance_links_election_candidate_idx
  ON public.sdcity_candidate_finance_links (election_id, candidate_id);
CREATE TRIGGER sdcity_candidate_finance_links_set_updated_at
  BEFORE UPDATE ON public.sdcity_candidate_finance_links
  FOR EACH ROW EXECUTE FUNCTION set_updated_at();

CREATE TABLE public.sdcity_candidate_finance_summaries (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE public.sdcity_candidate_finance_direct_breakdowns (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  link_id uuid NOT NULL,
  election_year integer NOT NULL CHECK (election_year BETWEEN 2000 AND 2100),
...
CREATE TRIGGER sdcity_candidate_finance_direct_breakdowns_set_updated_at
  BEFORE UPDATE ON public.sdcity_candidate_finance_direct_breakdowns
  FOR EACH ROW EXECUTE FUNCTION set_updated_at();
...
-- spenders coexist (the aggregator keys Pending spenders by name).
CREATE TABLE public.sdcity_candidate_finance_outside_groups (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  link_id uuid NOT NULL,
  election_year integer NOT NULL CHECK (election_year BETWEEN 2000 AND 2100),
...
CREATE TRIGGER sdcity_candidate_finance_outside_groups_set_updated_at
  BEFORE UPDATE ON public.sdcity_candidate_finance_outside_groups
  FOR EACH ROW EXECUTE FUNCTION set_updated_at();
