...
**Display path (2026-08-11 follow-up).** The shared ballot-lookup loader computes `total_raised = direct_contribution_total ?? total_receipts`, so storing the itemized sum in `direct_contribution_total` would show readers the itemized number, not the official YTD figure. Fix follows the Georgia pattern: the sync writes `direct_contribution_total = NULL` (itemized rows feed breakdowns only) and the loader discloses both boundaries via `directCoverageNote` (itemized breakdowns vs bank-YTD totals) and `outsideCoverageNote` (IEPAC-only outside totals).
