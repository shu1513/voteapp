...
- **Live-only, gated by the sync flag.** The batch pulls the grid (one 5,000-row page) and the all-time IE list (ONE page of 50,000 — the service returns unsorted rows, so it is never paged; 50,000 accepted live 2026-09-03, fail closed when `totalItems > items.length`) once per run, stores both as the **run artifact** (`scratch/idaho-campaign-finance/runs/<timestamp>.json` + sha256 manifest), and passes both to every link; the auto-link pass reuses the same grid pull (the grid promise is memoized, rejection included, so an outage costs one timeout). Idaho's raw-refresh flag gates nothing: the grid and the IE list are whole-dataset pulls, so no per-registration cache can feed a sync (the NH vendor twin is live-only too).
- **Per link:** the grid row must exist and carry the link's `electionYear` (else the link fails and nothing is written); the contribution search is fetched as ONE page of 10,000 by the grid's "First Middle Last" name and fails closed when `totalItems > items.length`; `aggregateIdahoContributions` (grid totals + row breakdowns) and `aggregateIdahoOutsideSpending` (entity + office + window) run; then the registration artifact is stored (the grid row, the registration's own rows, and the IE rows targeting the linked guid — the cache contract admits only rows keyed to that guid; the run artifact holds the prior-registration and name-only rows the aggregator also counts, so run artifact + registration artifact reproduce the snapshot); then one `replaceIdahoCandidateFinanceSnapshot`.
- **Presence:** the summary is always written from the grid; breakdowns are always written from the rows (coverage reported in the batch log and result as `rowCoverage` + `directCoverageNote`, never enforced; the loader carries the static coverage note); outside totals are always the aggregator's (0 with no rows). **No partial write:** an IE list that cannot be pulled fails the batch before any link is written, so a summary's `last_synced_at` always dates both legs (a direct-only write with preserved outside totals would show stale outside money as freshly updated).
- **Batch:** a failed link is recorded and the batch continues; the CLI exits 1 when any link failed (North Dakota convention); defaults `maxCandidates` 25, `staleAfterDays` 7, lookback 90 + stale + 1 = 98 days (annual report due January 31, 67-6607), lookahead 730. `--dry-run` skips the auto-link pass and the run artifact (fleet convention).
