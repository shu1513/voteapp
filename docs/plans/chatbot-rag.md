...
**Phase 2 — LLM answers (canary). BUILT 2026-08-12 (code complete; launch steps open):** `backend/src/chatbot/llm/` adapter boundary (`generateAnswer` → `{answer, citations, refusalReason, usage}`; impl #1 = OpenAI Responses API with `store:false`, strict structured output, reasoning effort from env, hashed `safety_identifier`), `answer.ts` guarded step (server-side citation validation — only supplied chunk ids count, zero valid citations → cards; URL/markdown stripping; server-constructed source cards; model refusals serve the standard deterministic refusal copy), `limits.ts` (per-user daily cap via Redis INCR+TTL on an HMAC of the user id keyed by the LLM API key; durable atomic daily budget on `chatbot.daily_budget` with the two-statement reserve-then-reconcile transaction — reconcile needs `::bigint` casts, pg cannot infer `$3 - $2`; 24h exact answer cache keyed sha256(question_norm + scope_key + generation_id + model + prompt_version)), pre-retrieval cache read in `askService`, token columns in the question log, widget AI label ("AI-generated from our election data — may contain mistakes. Data current as of …") + content-report control on the first cited entity. Flags all default OFF (`CHATBOT_LLM_ENABLED`, `CHATBOT_LLM_API_KEY`, …; see render.yaml notes). Verified 2026-08-12 against a MOCK Responses server end-to-end (label + citation cards + URL stripping + cache hit + cap fallback + budget reconcile) and Phase 1 gates re-ran green (recall@5 94%, template/policy/clarify/refusal 100%). **Launch steps all completed 2026-08-12:** privacy policy 1.1 names OpenAI as a processor (shipped with the Phase 2 PR), live-call groundedness spot-check passed (20 samples incl. every loaded-premise case, zero fabricated facts; refusal precision 8/8), LLM answers enabled in prod. Ongoing canary = `npm run chatbot:report` (llm_failed / invalid_output / spend).
