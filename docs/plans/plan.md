...
**Probe family DONE (2026-09-05, follow-up PR).** `utils/cliFlags.ts` gains `readStrictRequiredFlagValue`, `readStrictRequiredPositiveIntegerFlag`, and `readStrictNonNegativeNumberFlag`; the fallback-argument shapes (`parsePositiveIntegerFlag(args, name, fallback)`, `parseNonNegativeNumberFlag(...)`) need no primitive: call sites read `readStrict…(args, name) ?? fallback`. Cohort: the 10 probe scripts whose reader hashed to the strict body (alaska, arizona, hawaii, illinois, louisiana, massachusetts, newYorkSoda, tennessee, vermont, washington). Behavior fix: positive-integer flags now reject digit-only values above 2^53 (pinned in the 9 tested scripts). Kept local: louisiana `parseNonNegativeNumberFlag` (strips `$,`) + `parseOptionalAmountFlag`, alaska `parseOptionalNonNegativeIntegerFlag`, illinois `parseDateFlag`/`assertOnlyKnownValueFlags`, newYorkSoda `parseBoundedIntegerFlag`/`assertKnownFlags`. **Raw-refresh `readValueFlags` family DONE (2026-09-05, follow-up PR).** `utils/cliFlags.ts` gains `readStrictFlagValues(argv, flag): string[]` (every occurrence, trimmed, `Missing <flag> value` on a bare/empty/`--`-followed occurrence); `readStrictFlagValue` is now expressed over it. The 19 raw-refresh scripts (18 `refresh*RawData*` + `probeCaliforniaCampaignFinanceRawData`) drop their multi-value loop and keep only the 6-line at-most-once singular (`string | undefined` contract feeds their local helpers). Three source flavors: 12 strict-trim (message changed from `Missing value for X` to `Missing X value`, 4 test pins updated), 3 no-trim (california ×2, colorado TRACER: values now trimmed and an empty inline value throws), 4 lax (alabama, newHampshire, northDakota, westVirginia: a bare trailing flag threw a TypeError and `--x --y` took `--y` as the value; both now throw `Missing X value`). Fixes pinned in the 7 affected tests. Phase 9 has no open families left.
