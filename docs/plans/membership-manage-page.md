...
- **PR 2 — amount change.** Migration (pending-change table + `stripe_synced_at` guard column), `changeMonthlyAmount` + apply-if-due, `invoice.upcoming` handler, notice email, `pending_amount_change` in the status, window + concurrency tests. Reviewed on its own because it carries the §17602(g)(2) timing logic.
