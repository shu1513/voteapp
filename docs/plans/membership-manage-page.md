...
- **PR 3 — frontend. BUILT 2026-09-05.** `/me/membership` page (`pages/MembershipPage.tsx`, route `me/membership`), Settings cleanup, `/support/member` + `/mission` retargets, success-banner copy, the URL/label/message flips listed under PR 1 (every email link, the general-portal return, the started-email wording "open Manage membership", `membership_exists` → "Change the amount under Manage membership."), tests. Ships the feature. Deviations, on purpose:
  - **Shared pieces live in `components/SupportCheckout.tsx`** (the renamed `MembershipSection.tsx`): `AmountForm` (now keyed by `inputId`, takes `unchangedCents` so re-saving the amount in force keeps Save disabled, and renders helper children), formatters, `Disclaimer`, `SupportCheckout`, `MembershipThanks`. The POST helpers stay in the page, not the api-client package: mobile does not call them, so a package export would be surface for nothing.
  - **The status query is the shared `lib/useMembershipStatus`** (extracted on main by the My Draft membership ask, #1151); it now exports `MEMBERSHIP_QUERY_KEY` and its `queryFn` passes the query `signal`.
  - **Mutations cancel the in-flight GET before installing their result** (`cancelQueries` then `setQueryData` in `onSuccess`); a refetch that started before the change cannot overwrite it.
  - **Cache installs and Stripe redirects run from `mutate()`-level callbacks** (review 2026-09-05): TanStack drops those once the component that started the mutation is gone, while `useMutation`'s own `onSuccess` fires regardless — so a delayed answer for an account that has since signed out can neither land in the shared `["me", "membership"]` cache the next account reads nor redirect whoever is on the page now to someone else's Checkout or portal session. Account-scoped query keys were rejected: every reader of the key would change for a race the unmount already resolves.
  - **Projected start date is computed client-side for the helper line** (`projectedAmountStart`, a mirror of the backend's: this period's end at ≥7 days, else the renewal after on `started_at`'s day, clamped); the backend's `starts_at` is what the plan line and the "Saved." confirmation show.
  - **Change amount is shown for `active` and `past_due`, hidden for `incomplete`, `unpaid`, and while canceling** — the backend's own refusals; a refused replacement under 7 days surfaces the 409 message inline. Cancel is hidden for `incomplete` and while canceling (Keep membership shows instead); Update payment method always shows for a member.
  - **Settings profile line**: `active` → thank-you + Manage membership; `incomplete` → "being set up" + Manage membership; `past_due`/`unpaid` → "didn't go through" + Fix payment. A non-member keeps the green Become button, and a lapsed supporter also gets a "Payment history" link next to it (the plan had the link replace the button; hiding the invitation from a lapsed member would be a regression).
  - `/support/member` and `/support/once` each carry one cross-link to the other kind ("Prefer a one-time contribution? Support once" / "Prefer to become an honorary member? Support monthly") so neither is a dead end. `/support/member`'s member branch: `incomplete` → "being set up", `active`/`past_due` → "already an honorary member", other nonterminal → "already have a membership", each with a Manage membership link.
  - Logged-out `/me/membership` links to `/login?next=%2Fme%2Fmembership` (the emails link straight to the page). The page ends with a "Back to Settings" link.
  - The Checkout consent text still says "Cancel anytime in account settings (Manage membership)" — Settings links there, so it stays true.
...
4. PR 3 → deploy api + ssr (the api carries the URL/wording flips; ssr carries the page).
