...
**Mobile app and push notifications.** If you use the Elections Simplified app for iPhone or Android and allow notifications, we store a push notification token for that device (an identifier issued by Apple, Google, or the Expo push service that lets us send notifications to that device, not to you by name), the device platform (iOS or Android), and when the token was last used. The token is linked to your account so notifications about candidates you follow reach your devices. Signing out on a device or deleting your account removes that device's token. The app stores your sign-in session on the device in the operating system's secure storage.
