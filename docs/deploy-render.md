...
1. **Render blueprint**: New → Blueprint → select the GitHub repo → Render
   ...
   SES IAM key pair, and `EDGE_SHARED_SECRET` (optional — leave blank for
   the legacy blind-trust behavior; if set it must be ≥ 32 chars and the
   same value must go into the Cloudflare Worker, see Platform notes).
