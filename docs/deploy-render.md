...
1. **Render blueprint**: New → Blueprint → select the GitHub repo → Render
   reads render.yaml. Enter the three `sync: false` secrets: the send-only
   SES IAM key pair, and `EDGE_SHARED_SECRET` (required in production —
   ...
   must go into the Cloudflare Worker, see Platform notes).
