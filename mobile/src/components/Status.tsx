...
// Port of frontend/src/components/Status.tsx.

export function LoadingNotice({ text = "Loading…" }: { text?: string }) {
  return <Text className="py-8 text-center text-ink-soft">{text}</Text>;
}
...
export function ErrorNotice({ error }: { error: unknown }) {
  ...
  return (
    <Text className="rounded-lg border border-rausch/40 bg-rausch/5 px-3 py-2 text-sm text-rausch-dark">
      {message}
    </Text>
  );
}
