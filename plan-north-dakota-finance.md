...
- **WAF blocks plain curl** (403; this network also added TLS interception noise, so the curl result is not clean evidence by itself). Presigned S3 URLs are WAF-free once obtained.
...
- Transaction API: `POST /api/Public-Service/CommitteeTransactions/getAllPublicTransactionDataList`. **Open contract problem (probed hard, unsolved):** a flat `{pageNumber,pageSize}` body returns the Expenditures dataset; the dataset-selector member is unknown — `transactionTypeCode` is accepted but every guessed value returns 0 rows, and the SPA's request bodies were not interceptable. Phase 0A pins this or the design uses the documented fallbacks (gate 2).
...
- **Rows are per-candidate allocations (verified arithmetically — corrected from the first probe's "repeated payment" reading):** StrongND Fund's rows are 7 × $16,857.14 + 18 × $2,000 = **$153,999.98 = the filing's `transactionTotalYTD` exactly**, and each row has its own sequential `transactionID`. The row amounts are candidate-level allocations (7 × $16,857.14 is an even split of a $117,999.98 buy), so **summing rows once per row identity is the correct committee total**. Rules: sum unique `transactionID`s; never dedupe by spender+vendor+date+amount (equal slate allocations are legitimate); `transactionTotalYTD` is a reconciliation control, never a spend row. Phase 0A gate: confirm `transactionID` uniqueness/stability and that repeated IDs (if ever observed) mean amendment lineage, not allocation.
...
- **No IE bulk CSV, and no export control on the public IE grid (checked 2026-08-26)** — acquisition is the critical gate (Phase 0A gate 2).
...
- **Phase 0A — probe now (no schema, no publication).** `npm run north-dakota-candidates:finance:phase-zero`. Hard gates:
  1. **Production transport proven.** Registry, presign, CSV download, contribution rows, and IE rows fetched from the actual runtime (local Node first, Render-equivalent network if scheduled sync is ever planned there) — not browser context. If Node fetch fails the WAF, stop and pick the acquisition model (SOS feed, or local-acquisition-promote-later like Missouri) before writing the client.
  2. **IE acquisition pinned.** In order: (a) re-check the grids for an export control (none visible 2026-08-26, but the bundle ships an `exportHandler` chunk); (b) find the transaction API's dataset-selector member and replay it headlessly (row count matches the portal); (c) else choose the documented fallback — SOS-provided export or per-filing PDFs with the IE chart endpoint as control. A permanent 0-row response must be distinguishable from "filter silently ignored".
