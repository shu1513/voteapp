  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <h3 className="font-semibold text-gray-900">{election.official_ballot_title}</h3>
        <span className="shrink-0 text-sm text-gray-600">{formatElectionDate(election.election_date)}</span>
      </div>
      ...
      <div className="mt-2 flex flex-wrap items-center gap-2 text-xs">
        {election.race_type === "ballot_measure" ? (
          <span className="rounded bg-purple-100 px-2 py-0.5 text-purple-800">Ballot measure</span>
        ) : (
        ...
        {election.has_results ? (
          <span className="rounded bg-green-100 px-2 py-0.5 text-green-800">Results available</span>
        ) : null}
        {election.research_areas.map((area) => (
          <span key={area.id} className="rounded bg-gray-100 px-2 py-0.5 text-gray-600">
            {area.name}
          </span>
        ))}
      </div>
