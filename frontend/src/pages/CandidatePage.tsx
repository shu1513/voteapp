...
export function CandidatePage() {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8">
      ...
      {candidate.summary ? <p className="mt-3 text-gray-800">{candidate.summary}</p> : null}
        ...
        <section className="mt-6">
          ...
          {recordGroups.map((group) => (
            <div key={group.areaName} className="mt-4">
              <h3 className="text-sm font-semibold uppercase tracking-wide text-gray-500">{group.areaName}</h3>
              <ul className="mt-2 space-y-3">
                {group.records.map((record) => (
                  <li key={`${group.areaName}-${record.id}`} className="rounded-lg border border-gray-200 bg-white p-3">
                    <p className="text-sm text-gray-800">{record.description}</p>
                    <p className="mt-1 text-xs text-gray-500">{formatElectionDate(record.event_date)}</p>
          ...
          <ul className="mt-2 divide-y rounded-lg border border-gray-200 bg-white">
            {candidate.elections.map((election) => (
              <li key={election.candidate_election_id} className="px-3 py-2 text-sm">
                <Link to={`/elections/${election.election_id}`} className="text-blue-700 underline">
