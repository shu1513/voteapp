...
- **Both sources miss filings**: Stiens HD39's 202607 + PLF scans are in the tree but absent from the viewer's R&E grid; Muter HD2's 1/13/2026 paper row is in the viewer while the tree shows N/A for 202601. Rule: tree links give the periods (a scan is a filing); the candidate is complete only when the tree explains at least every viewer paper row (`viewer rows <= tree versions due inside the window + in-window PLF/GLF scans`, no unmapped filenames — the viewer lists a paper last-minute report as a report row too).
...
- **Schedule B rows (in-kind) — DONE 2026-09-03**: `parseKansasScheduleBRows` + `parseKansasScheduleBTotals` + `checkKansasScheduleB`. Live shape (five Governor 2026 reports, 27 rows): five cells — date, name+address (same `Repeater2_lblAddress_N`/`lblZip_N` anchors as Schedule A; zip may carry +4), occupation, description ("Food and Drink", "Donation of Signs"), value — no tender column, no running phase totals. Every captured page was cent-exact: row sum = `lblTotalItemized`, itemized + unitemized = `lblTotalInKind`, and `lblTotalInKind` = cover line 6 on all five. Filers itemize under the $100 line too ($50 rows), so the sum check, not the threshold, is the gate.

Remaining: Schedule C rows (only for the Phase 5 PAC path); KPDC PDF classification (cover recovery exists from Phase 0; itemized rows from OCR are out of scope — paper filers get totals only and no breakdowns).
...
### Phase 4 — direct side (first shippable release) — IN PROGRESS
...
- **Step 1 — cover totals — DONE 2026-09-03**: `aggregateKansasCoverTotals` (kansasDirectContributionAggregator.ts), pure. Sums cover lines 2 / 4 / 6 over the CANONICAL version of every counted period (`report_filed | amended`), cash on hand = line 5 of the latest canonical report (null when the close is negative — the summaries table rejects negatives). A cycle with no filed report (affidavit-exempt, or first report not yet due) is `no_filed_report`, never $0; exempt periods beside filed ones are a diagnostic (under $1,000 each, unreported). Line 6 stays separate from line 2 (the form keeps in-kind out of receipts). Fail closed per candidate: ledger incomplete, a cover failing 1+2=3 / 3−4=5, an unparsed line 6, a canonical version with no opened cover (paper scans until the OCR step), or two opened covers matching one canonical version (matched by `kansasFilingHeaderKey`, the ledger's value identity — every ordering field incl. the KPDC ordinal, so one key = a tie = never canonical). Balance continuity (period N line 1 = period N−1 line 5) is a diagnostic, not a gate. Live 2026-09-03, five Governor 2026 filers: all publishable, zero continuity diagnostics, Colyer's amended → terminated → reopened chain summed from canonical versions only; Eakins terminated to a $0.00 close. Next: step 2 breakdowns (Schedule A/B rows → occupation + size buckets + coverage), then writer/sync CLI.
