...
Remaining: wire the ledger into the sync (e-filed period from the cover, paper period from the KPDC filename token, AT/affidavit dates from the Phase 3 enumeration); Schedule B rows (in-kind individuals) and Schedule C rows (only for the Phase 5 PAC path); KPDC PDF classification (cover recovery exists from Phase 0; itemized rows from OCR are out of scope — paper filers get totals only and no breakdowns).
