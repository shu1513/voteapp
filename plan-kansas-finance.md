...
Remaining: paper inventory from the KPDC index (House tree `House/<year>ElecCycle/HLinks<year>EC.htm`; ledger headers will need date-less versions ordered by the amend prefix); Schedule B rows (in-kind individuals) and Schedule C rows (only for the Phase 5 PAC path); KPDC PDF classification (cover recovery exists from Phase 0; itemized rows from OCR are out of scope — paper filers get totals only and no breakdowns).
