...
- **Resolver rules** (fail closed): exact district for districted offices (statewide ignores the column); full-name evidence through the shared middle-name gate with one-sided roster->filing nickname expansion (`Steve`->`STEVEN`, `Chris`->`CHRISTOPHER`); every aligned spelling is one person unless two aligned spellings contradict each other on middle names or generational suffix (-> `ambiguous`, no link); blank-district-only alignments -> `manual_confirm_required`; a bare surname never links. County rows (`county::District Attorney` = County Attorneys) and federal races are out of the office map.
