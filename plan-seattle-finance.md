...
- 2026 Seattle races already in our DB and in PDC (live figures as of the 2026-08-10 planning snapshot; filings continue — e.g. Rothstein rose to $37,003.29 by the 2026-08-11 acceptance run): Council District 5 (Kang $112,466.50 / $96,123.52; Jenks $111,998.36 / $105,623.51; James $49,298.56 / $27,017.87; Georgakopoulos $2,050.00 / $1,800.00) and Municipal Court Judge Position 5 (Newsom $82,994.85; Rothstein $35,603.29; Calkins $8,372.90). Council rows: `office='CITY COUNCIL MEMBER'`, `jurisdiction='CITY OF SEATTLE'`, `position='5'`. Judge rows: `office='MUNICIPAL COURT JUDGE'`, `jurisdiction='SEATTLE MUNICIPAL COURT'`, positions 1–7.
...
## Phase 4: rollout — DONE (2026-08-11)
...
- ✅ "Raised" semantics contract — VERIFIED LIVE 2026-08-11: Wilson 2025 itemized cash ($1,018,974.65) + in-kind ($5,984.88) sums cent-exact to summary `contributions_amount` ($1,024,959.53); small-contribution batches (code "Other", where democracy vouchers land) included; `loans_amount` is a separate summary field the sync never reads. The `totalReceipts = directContributionTotal = contributionsAmount` pass-through was already pinned by fixture in `washingtonCandidateFinanceSync.test.ts`; semantics now documented at `toSummary` in the sync.
- ✅ Legal release gate — REMOVED by owner decision 2026-08-11 after risk analysis. Basis: RCW 42.56.070(9) restricts providing "lists of individuals" for commercial purposes; AGO 1975 No. 15 reads "commercial purposes" as contacting/personally affecting the listed individuals for commercial gain. VoteApp stores and displays no individual contributor names from WA data — direct side is server-side occupation/size aggregates only, and the funders query hard-filters `contributor_category = 'Organization'` in `washingtonPdcClient.ts` (verified in stored rows: all WA donor names are organizations/PACs). Production enablement no longer blocked. Standing rule stays: aggregates only, never add WA donor-name display without revisiting this analysis (see Non-goals).
- ✅ BatchSync place support — shipped in Phase 3 (#648): due-list query carries place office keys; auto-link derives jurisdiction/position; resyncs ride trusted hard IDs.
- ✅ Tests — shipped in Phase 3 (#648): eligible-offices mapping, resolver place cases, IE fixtures, coverage note.
- ✅ End-to-end acceptance — RUN 2026-08-11 local, PASSED: `washington-candidates:finance:sync-due --lookback-days 8` auto-linked and synced all 7 rostered Seattle candidates (Council D5: Kang $112,466.50, Jenks $111,998.36 + IE-for $167.75, James $49,298.56, Georgakopoulos $2,050.00; Municipal Court P5: Newsom $82,994.85, Rothstein $37,003.29, Calkins $8,372.90) — every total cent-exact vs same-day PDC summary rows, `total_receipts = direct_contribution_total` on all 7, links store district=SEATTLE, occupation/size breakdowns populated. 3 auto-link warnings were pre-existing unlinked legislative primary candidates with no PDC committee (`no_candidate_committee_match`), not place-scope rows.
