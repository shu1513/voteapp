...
- Industry classification uses deterministic finance-label rules plus the manually researched `finance_label_classifications` rows. The manual due-sync CLI never calls AI; only scheduler-triggered jobs still accept `--ai-classify-industries`.
