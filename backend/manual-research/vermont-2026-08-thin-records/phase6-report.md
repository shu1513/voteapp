...
`manual:candidate-records:write` does not delete stale records by itself, so after the verified rerun a narrow cleanup removed the six exact old candidacy-only rows, one nominee-status row, and their tags:
