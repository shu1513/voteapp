...
(async () => {
  ...
  const runId = `same_day_live_ai_${new Date().toISOString()}`;
  ...
  try {
    ...
    const states = await pool.query<StateRow>(
      `SELECT state_fips, state_abbreviation, state_name FROM state_resources ORDER BY state_fips LIMIT 51`
    );

    if (states.rowCount !== 51) {
      throw new Error(`Expected 51 states in state_resources, found ${states.rowCount ?? 0}`);
    }
    ...
    for (let round = 1; round <= maxRounds; round += 1) {
      ...
      const done = status.written === 51 && status.pending === 0 && status.validated === 0;
