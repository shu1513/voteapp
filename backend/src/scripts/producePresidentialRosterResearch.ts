import { runPresidentialRosterResearchProducer } from "../pipeline/producers/presidentialRosterResearchProducer.js";
...
async function main(): Promise<void> {
  ...
  console.log(JSON.stringify({ type: "presidential_roster_research_producer", ...result }, null, 2));
}

main().catch((error) => {
  const message = error instanceof Error ? error.message : String(error);
  console.error("presidential roster research producer failed:", message);
  process.exitCode = 1;
});
