...
import {
  enqueueManualNorthCarolinaCandidateFinanceSyncJob,
...
export function parseNorthCarolinaCandidateFinanceSyncTriggerArgs(
...
async function main(): Promise<void> {
  loadProjectEnv();
  const jobData = parseNorthCarolinaCandidateFinanceSyncTriggerArgs(process.argv.slice(2));
  const jobId = await enqueueManualNorthCarolinaCandidateFinanceSyncJob(jobData);
  if (jobId === "disabled") {
    console.log("North Carolina campaign finance sync is disabled; job was not enqueued");
    return;
  }
  console.log(
    `North Carolina campaign finance sync job enqueued (jobId=${jobId} force=${Boolean(
      jobData.force
...
if (entrypoint === import.meta.url) {
  main().catch((error) => {
    console.error("North Carolina campaign finance sync trigger failed:", error);
    process.exit(1);
  });
}
