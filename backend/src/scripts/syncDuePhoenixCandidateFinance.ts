...
import { isPhoenixCampaignFinanceSyncEnabled } from "../config/featureFlags.js";
import {
  syncDuePhoenixCandidateFinance,
  type PhoenixCandidateFinanceBatchSyncResult,
} from "../pipeline/phoenixFinance/phoenixCandidateFinanceBatchSync.js";

export type SyncDuePhoenixCandidateFinanceScriptOptions = {
  dryRun: boolean;
  force: boolean;
...
export function parseSyncDuePhoenixCandidateFinanceScriptArgs(
  args: readonly string[],
): SyncDuePhoenixCandidateFinanceScriptOptions {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    throw new Error(`Unknown Phoenix candidate finance flag: ${arg}`);
  }
  return {
...
export function toSyncDuePhoenixCandidateFinanceScriptOutput(input: {
  startedAt: Date;
  options: SyncDuePhoenixCandidateFinanceScriptOptions;
  result: PhoenixCandidateFinanceBatchSyncResult;
}) {
  return {
    type: "phoenix_candidate_finance_due_sync",
...
async function main(): Promise<void> {
  ...
  loadProjectEnv();
  const options = parseSyncDuePhoenixCandidateFinanceScriptArgs(
    process.argv.slice(2),
  );

  if (!isPhoenixCampaignFinanceSyncEnabled(options.force)) {
  ...
  try {
    ...
    console.log(
      JSON.stringify(
        toSyncDuePhoenixCandidateFinanceScriptOutput({
          startedAt,
          options,
          result,
        }),
...
if (entrypoint === import.meta.url) {
  main().catch((error) => {
    ...
    console.error("Phoenix candidate finance due sync failed:", message);
    process.exitCode = 1;
  });
}
