...
async function main(): Promise<void> {
  loadProjectEnv();
  const jobData = parseHoustonCandidateFinanceSyncTriggerArgs(process.argv.slice(2));
  const jobId = await enqueueManualHoustonCandidateFinanceSyncJob(jobData);
  if (jobId === "disabled") {
    console.log("Houston campaign finance sync is disabled; job was not enqueued");
    return;
  }
  console.log(
    `Houston campaign finance sync job enqueued (jobId=${jobId} force=${Boolean(
      jobData.force
...
if (entrypoint === import.meta.url) {
  main().catch((error) => {
    console.error("Houston campaign finance sync trigger failed:", error);
    process.exit(1);
  });
}
