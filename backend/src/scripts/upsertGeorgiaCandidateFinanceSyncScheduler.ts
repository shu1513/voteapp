...
import {
  upsertRecurringGeorgiaCandidateFinanceSyncJobs,
...
async function main(): Promise<void> {
  loadProjectEnv();
  const jobData = parseUpsertGeorgiaCandidateFinanceSyncSchedulerArgs(process.argv.slice(2));
  const enabled = isGeorgiaCampaignFinanceEnabled();
  await upsertRecurringGeorgiaCandidateFinanceSyncJobs(jobData);
  console.log(
    enabled
      ? "Georgia campaign finance recurring scheduler upserted (daily sync)"
      : "Georgia campaign finance recurring scheduler disabled; scheduler cleanup completed"
  );
}
...
if (entrypoint === import.meta.url) {
  main().catch((error) => {
    console.error("Georgia campaign finance recurring scheduler upsert failed:", error);
    process.exit(1);
  });
}
