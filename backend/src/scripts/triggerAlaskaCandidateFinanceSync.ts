...
import {
  enqueueManualAlaskaCandidateFinanceSyncJob,
...
export function parseAlaskaCandidateFinanceSyncTriggerArgs(args: readonly string[]): AlaskaCandidateFinanceSyncJobData {
  return {
...
async function main(): Promise<void> {
  loadProjectEnv();
  const jobData = parseAlaskaCandidateFinanceSyncTriggerArgs(process.argv.slice(2));
  const jobId = await enqueueManualAlaskaCandidateFinanceSyncJob(jobData);
  if (jobId === "disabled") {
    console.log("Alaska campaign finance sync is disabled; job was not enqueued");
    return;
  }
  console.log(
    `Alaska campaign finance sync job enqueued (jobId=${jobId} force=${Boolean(jobData.force)} dryRun=${jobData.dryRun !== false})`
  );
}
...
if (entrypoint === import.meta.url) {
  main().catch((error) => {
    console.error("Alaska campaign finance sync trigger failed:", error);
    process.exit(1);
  });
}
