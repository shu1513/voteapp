...
export function parseHawaiiCandidateFinanceSyncTriggerArgs(
  args: readonly string[]
): HawaiiCandidateFinanceSyncJobData {
  assertKnownCliFlags(args, "Hawaii candidate finance sync", KNOWN_BOOLEAN_FLAGS, KNOWN_VALUE_FLAGS);
  return {
...
async function main(): Promise<void> {
  loadProjectEnv();
  const jobData = parseHawaiiCandidateFinanceSyncTriggerArgs(process.argv.slice(2));
  const jobId = await enqueueManualHawaiiCandidateFinanceSyncJob(jobData);
  if (jobId === "disabled") {
    console.log("Hawaii campaign finance sync is disabled; job was not enqueued");
    return;
  }
  console.log(
    `Hawaii campaign finance sync job enqueued (jobId=${jobId} force=${Boolean(jobData.force)} dryRun=${Boolean(
      jobData.dryRun
    )})`
  );
}
...
if (entrypoint === import.meta.url) {
  main().catch((error) => {
    console.error("Hawaii campaign finance sync trigger failed:", error);
    process.exit(1);
  });
}
