...
import {
  isSanDiegoCampaignFinanceRawDataRefreshEnabled,
  isSanDiegoCampaignFinanceSyncEnabled,
} from "../config/featureFlags.js";
import {
  syncDueSanDiegoCityCandidateFinance,
  type SanDiegoCityCandidateFinanceBatchSyncResult,
} from "../pipeline/sanDiegoCityFinance/sanDiegoCityCandidateFinanceBatchSync.js";

export type SyncDueSanDiegoCityCandidateFinanceScriptOptions = {
  dryRun: boolean;
  force: boolean;
...
export function parseSyncDueSanDiegoCityCandidateFinanceScriptArgs(
  args: readonly string[],
): SyncDueSanDiegoCityCandidateFinanceScriptOptions {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    throw new Error(`Unknown San Diego candidate finance flag: ${arg}`);
  }
  return {
...
export function toSyncDueSanDiegoCityCandidateFinanceScriptOutput(input: {
  startedAt: Date;
  options: SyncDueSanDiegoCityCandidateFinanceScriptOptions;
  rawDataRefreshEnabled: boolean;
  result: SanDiegoCityCandidateFinanceBatchSyncResult;
}) {
  return {
    type: "san_diego_candidate_finance_due_sync",
...
async function main(): Promise<void> {
  ...
  loadProjectEnv();
  const options = parseSyncDueSanDiegoCityCandidateFinanceScriptArgs(
    process.argv.slice(2),
  );

  if (!isSanDiegoCampaignFinanceSyncEnabled(options.force)) {
  ...
  const rawDataRefreshEnabled = isSanDiegoCampaignFinanceRawDataRefreshEnabled(
    options.force,
  );

  const pool = new Pool({ connectionString: getPipelineEnv().DATABASE_URL });
  try {
    const result = await syncDueSanDiegoCityCandidateFinance({
      db: pool,
      now: startedAt,
    ...
    console.log(
      JSON.stringify(
        toSyncDueSanDiegoCityCandidateFinanceScriptOutput({
          startedAt,
          options,
...
if (entrypoint === import.meta.url) {
  main().catch((error) => {
    ...
    console.error("San Diego candidate finance due sync failed:", message);
    process.exitCode = 1;
  });
}
