...
export function parseUtahCandidateFinanceSyncTriggerArgs(
  args: readonly string[]
): UtahCandidateFinanceSyncJobData {
  assertKnownCliFlags(args, "Utah candidate finance sync", KNOWN_BOOLEAN_FLAGS, KNOWN_VALUE_FLAGS);
  return {
...
async function main(): Promise<void> {
  loadProjectEnv();
  const jobData = parseUtahCandidateFinanceSyncTriggerArgs(process.argv.slice(2));
  const jobId = await enqueueManualUtahCandidateFinanceSyncJob(jobData);
  if (jobId === "disabled") {
    console.log("Utah campaign finance sync is disabled; job was not enqueued");
    return;
  }
  console.log(
    `Utah campaign finance sync job enqueued (jobId=${jobId} force=${Boolean(jobData.force)} dryRun=${Boolean(
      jobData.dryRun
    )})`
  );
}
...
if (entrypoint === import.meta.url) {
  main().catch((error) => {
    console.error("Utah campaign finance sync trigger failed:", error);
    process.exit(1);
  });
}
