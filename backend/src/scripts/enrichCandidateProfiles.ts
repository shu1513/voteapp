import { runCandidateProfileEnricher } from "../pipeline/enrichers/candidateProfileEnricher.js";
...
async function main(): Promise<void> {
  ...
  await runCandidateProfileEnricher({ once, batchSize, blockMs });
}

main().catch((error) => {
  const message = error instanceof Error ? error.message : String(error);
  console.error("candidate-profile enricher failed:", message);
  process.exitCode = 1;
});
