...
async function main(): Promise<void> {
  loadProjectEnv();
  const jobData = parseMinnesotaCandidateFinanceSyncTriggerArgs(process.argv.slice(2));
  const jobId = await enqueueManualMinnesotaCandidateFinanceSyncJob(jobData);
  if (jobId === "disabled") {
    console.log("Minnesota campaign finance sync is disabled; job was not enqueued");
    return;
  }
  console.log(
    `Minnesota campaign finance sync job enqueued (jobId=${jobId} force=${Boolean(jobData.force)} dryRun=${Boolean(
      jobData.dryRun
    )})`
  );
}
...
if (entrypoint === import.meta.url) {
  main().catch((error) => {
    console.error("Minnesota campaign finance sync trigger failed:", error);
    process.exit(1);
  });
}
