import { runBallotMeasuresEnricher } from "../pipeline/enrichers/ballotMeasuresEnricher.js";
...
async function main(): Promise<void> {
  ...
  await runBallotMeasuresEnricher({ once, batchSize, blockMs });
}

main().catch((error) => {
  const message = error instanceof Error ? error.message : String(error);
  console.error("ballot-measures enricher failed:", message);
  process.exitCode = 1;
});
