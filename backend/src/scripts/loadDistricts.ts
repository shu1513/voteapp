...
function readTypeArg(argv: string[]): DistrictLoadType {
  ...
  if (value === "state" || value === "us_house" || value === "county" || value === "incorporated_place") {
    return value;
  }

  throw new Error(`Invalid --type value: ${value}. Supported: state, us_house, county, incorporated_place`);
}
