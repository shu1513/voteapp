...
export function parseSyncDueFloridaCandidateFinanceScriptArgs(
  ...
  return {
    ...
    staleAfterDays: parseIntegerFlag({ args, name: "--stale-after-days" }),
    electionLookbackDays: parseIntegerFlag({ args, name: "--lookback-days" }),
    electionLookaheadDays: parseIntegerFlag({ args, name: "--lookahead-days" }),
