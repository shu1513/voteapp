...
export function parseArizonaCandidateFinanceSyncTriggerArgs(
...
async function main(): Promise<void> {
  loadProjectEnv();
  const jobData = parseArizonaCandidateFinanceSyncTriggerArgs(process.argv.slice(2));
  const jobId = await enqueueManualArizonaCandidateFinanceSyncJob(jobData);
  if (jobId === "disabled") {
    console.log("Arizona campaign finance sync is disabled; job was not enqueued");
    return;
  }
  console.log(
    `Arizona campaign finance sync job enqueued (jobId=${jobId} force=${Boolean(jobData.force)} dryRun=${Boolean(
      jobData.dryRun
    )})`
  );
}
...
if (entrypoint === import.meta.url) {
  main().catch((error) => {
    console.error("Arizona campaign finance sync trigger failed:", error);
    process.exit(1);
  });
}
