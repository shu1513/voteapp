...
export function parseMarylandCandidateFinanceSyncTriggerArgs(
  args: readonly string[]
): MarylandCandidateFinanceSyncJobData {
  assertKnownCliFlags(args, "Maryland candidate finance sync", KNOWN_BOOLEAN_FLAGS, KNOWN_VALUE_FLAGS);
  return {
...
async function main(): Promise<void> {
  loadProjectEnv();
  const jobData = parseMarylandCandidateFinanceSyncTriggerArgs(process.argv.slice(2));
  const jobId = await enqueueManualMarylandCandidateFinanceSyncJob(jobData);
  if (jobId === "disabled") {
    console.log("Maryland campaign finance sync is disabled; job was not enqueued");
    return;
  }
  console.log(
    `Maryland campaign finance sync job enqueued (jobId=${jobId} force=${Boolean(jobData.force)} dryRun=${Boolean(
      jobData.dryRun
    )})`
  );
}
...
if (entrypoint === import.meta.url) {
  main().catch((error) => {
    console.error("Maryland campaign finance sync trigger failed:", error);
    process.exit(1);
  });
}
