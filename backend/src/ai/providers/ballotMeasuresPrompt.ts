...
export function buildBallotMeasuresPrompt(input: BallotMeasurePromptInput): string {
  ...
  return [
    ...
    "- If the measure has a cost, include the money in the summary: what it costs the government to do (for example, total bond amount or project cost) and what it costs taxpayers (for example, tax rate change or estimated cost per household per year), using the official fiscal estimate when one exists.",
