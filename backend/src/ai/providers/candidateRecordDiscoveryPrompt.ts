...
export function buildCandidateRecordDiscoveryPrompt(input: CandidateRecordDiscoveryPromptInput): string {
  ...
  return [
    ...
    ...(input.sinceDate
      ...
      : []),
    "- Return the comprehensive set of substantive records across those categories, proportionate to the candidate's actual public career — do not stop at one or two representative records for a documented public figure. There is no target number of records. Substantive means it tells a voter something about how the candidate governs: skip routine administrative items (minutes approvals, small routine contracts, ceremonial resolutions) even when the vote was split, unless the candidate's vote or role there genuinely reveals a priority or stance.",
