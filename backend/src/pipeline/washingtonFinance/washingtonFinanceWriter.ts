...
export type WashingtonFinanceLinkStatus = "active" | "inactive";
export type WashingtonFinanceLinkSource = "manual" | "pdc_api";
export type WashingtonFinanceDirectCategoryType = "occupation" | "contribution_size";
export type WashingtonFinanceOutsideCategoryType = "donor" | "industry";
export type WashingtonFinanceSupportOppose = "support" | "oppose";

export type WashingtonFinanceLinkInput = {
  candidateId: string;
  electionId: string;
  electionYear: number;
  ...
  linkStatus?: WashingtonFinanceLinkStatus;
  linkSource?: WashingtonFinanceLinkSource;
  sourceUrl?: string | null;
  lastVerifiedAt?: Date | null;
};

export type WashingtonFinanceSummaryInput = {
...
export type WashingtonFinanceDirectBreakdownInput = {
  categoryType: WashingtonFinanceDirectCategoryType;
  categoryName: string;
  amount: number;
...
export type WashingtonFinanceOutsideGroupInput = {
  sponsorId: string;
  sponsorName: string;
  supportOppose: WashingtonFinanceSupportOppose;
  amount: number;
  sourceUrl?: string | null;
};

export type WashingtonFinanceOutsideGroupBreakdownInput = {
  sponsorId: string;
  supportOppose: WashingtonFinanceSupportOppose;
  categoryType: WashingtonFinanceOutsideCategoryType;
  categoryName: string;
  amount: number;
  contributorCount?: number | null;
  sourceUrl?: string | null;
...
export type WashingtonFinanceSnapshotInput = {
  ...
  directBreakdowns?: readonly WashingtonFinanceDirectBreakdownInput[];
  outsideGroups?: readonly WashingtonFinanceOutsideGroupInput[];
  outsideGroupBreakdowns?: readonly WashingtonFinanceOutsideGroupBreakdownInput[];
  classifications?: readonly FinanceLabelClassification[];
};

export type WashingtonFinanceSnapshotWriteResult = {
  linkId: string;
...
function normalizeNullableDate(value: Date | null | undefined): string | null {
  ...
  if (Number.isNaN(value.getTime())) {
    throw new Error("Invalid Washington finance timestamp");
  }
...
function normalizeNullableCount(value: number | null | undefined): number | null {
  ...
  if (!Number.isInteger(value) || value < 0) {
    throw new Error("Washington finance contributor count must be a nonnegative integer");
  }
  return value;
}
...
function validateWashingtonFinanceLinkInput(link: WashingtonFinanceLinkInput): void {
  ...
  requireNonEmpty(link.candidateNameNormalized, "Washington finance candidate name");
  requireNonEmpty(link.officeName, "Washington finance office name");
  requireNonEmpty(link.filerId, "Washington filer id");
  requireNonEmpty(link.committeeId, "Washington committee id");
  requireNonEmpty(link.committeeName, "Washington committee name");
...
export async function upsertWashingtonFinanceLink(input: {
  db: Queryable;
  link: WashingtonFinanceLinkInput;
  ...
  const result = await input.db.query<ManualProtectedLinkRow>(
    `
      INSERT INTO public.wa_candidate_finance_links (
        candidate_id,
        election_id,
        election_year,
      ...
      DO UPDATE SET
        ...
        ${manualProtectedLinkAssignments("wa_candidate_finance_links")},
    ...
    [
      ...
      requireNonEmpty(input.link.candidateNameNormalized, "Washington finance candidate name"),
      requireNonEmpty(input.link.officeName, "Washington finance office name"),
      normalizeOptionalText(input.link.district),
      requireNonEmpty(input.link.filerId, "Washington filer id"),
      requireNonEmpty(input.link.committeeId, "Washington committee id"),
      requireNonEmpty(input.link.committeeName, "Washington committee name"),
      normalizeOptionalText(input.link.candidacyId),
  ...
  assertLinkWriteNotBlocked("Washington", result.rows[0], input.link.linkSource ?? "manual", input.link.electionYear);
  const linkId = result.rows[0]?.id;
  if (!linkId) {
    throw new Error("Washington finance link upsert did not return an id");
  }
  return { linkId };
}
...
async function upsertSummary(input: {
  ...
  await input.db.query(
    `
      INSERT INTO public.wa_candidate_finance_summaries (
        link_id,
        election_year,
        total_receipts,
    ...
    `,
    [
      requireNonEmpty(input.linkId, "Washington finance link id"),
...
async function upsertDirectBreakdown(input: {
  ...
  linkId: string;
  electionYear: number;
  breakdown: WashingtonFinanceDirectBreakdownInput;
  syncedAt: Date;
}): Promise<void> {
  await input.db.query(
    `
      INSERT INTO public.wa_candidate_finance_direct_breakdowns (
        link_id,
        election_year,
        category_type,
        category_name,
    ...
    [
      ...
      requireNonEmpty(input.breakdown.categoryName, "Washington finance direct breakdown category"),
...
async function upsertOutsideGroup(input: {
  ...
  linkId: string;
  electionYear: number;
  group: WashingtonFinanceOutsideGroupInput;
  syncedAt: Date;
}): Promise<void> {
  await input.db.query(
    `
      INSERT INTO public.wa_candidate_finance_outside_groups (
        link_id,
        election_year,
        sponsor_id,
        sponsor_name,
    ...
    [
      requireNonEmpty(input.linkId, "Washington finance link id"),
      normalizeElectionYear(input.electionYear),
      requireNonEmpty(input.group.sponsorId, "Washington outside group sponsor id"),
      requireNonEmpty(input.group.sponsorName, "Washington outside group sponsor name"),
...
async function upsertOutsideGroupBreakdown(input: {
  ...
  await input.db.query(
    `
      INSERT INTO public.wa_candidate_finance_outside_group_breakdowns (
        link_id,
        election_year,
        sponsor_id,
        support_oppose,
    ...
    [
      requireNonEmpty(input.linkId, "Washington finance link id"),
      normalizeElectionYear(input.electionYear),
      requireNonEmpty(input.breakdown.sponsorId, "Washington outside breakdown sponsor id"),
      input.breakdown.supportOppose,
      input.breakdown.categoryType,
      requireNonEmpty(input.breakdown.categoryName, "Washington outside breakdown category"),
...
async function deleteStaleDirectBreakdowns(input: {
  ...
  linkId: string;
  electionYear: number;
  breakdowns: readonly WashingtonFinanceDirectBreakdownInput[];
}): Promise<void> {
  const keys = input.breakdowns.map((breakdown) => ({
    category_type: breakdown.categoryType,
    category_name: requireNonEmpty(breakdown.categoryName, "Washington finance direct breakdown category"),
  }));

  await input.db.query(
    `
      DELETE FROM public.wa_candidate_finance_direct_breakdowns
      WHERE link_id = $1::uuid
        ...
        AND NOT EXISTS (
          ...
          WHERE keep.category_type = wa_candidate_finance_direct_breakdowns.category_type
            AND keep.category_name = wa_candidate_finance_direct_breakdowns.category_name
        )
    `,
    [requireNonEmpty(input.linkId, "Washington finance link id"), normalizeElectionYear(input.electionYear), JSON.stringify(keys)]
  );
}
...
async function deleteStaleOutsideGroupBreakdowns(input: {
  ...
  const keys = input.breakdowns.map((breakdown) => ({
    sponsor_id: requireNonEmpty(breakdown.sponsorId, "Washington outside breakdown sponsor id"),
    support_oppose: breakdown.supportOppose,
    category_type: breakdown.categoryType,
    category_name: requireNonEmpty(breakdown.categoryName, "Washington outside breakdown category"),
  }));

  await input.db.query(
    `
      DELETE FROM public.wa_candidate_finance_outside_group_breakdowns
      WHERE link_id = $1::uuid
        AND election_year = $2
        AND NOT EXISTS (
          SELECT 1
          ...
          WHERE keep.sponsor_id = wa_candidate_finance_outside_group_breakdowns.sponsor_id
            AND keep.support_oppose = wa_candidate_finance_outside_group_breakdowns.support_oppose
            AND keep.category_type = wa_candidate_finance_outside_group_breakdowns.category_type
            AND keep.category_name = wa_candidate_finance_outside_group_breakdowns.category_name
        )
    `,
    [requireNonEmpty(input.linkId, "Washington finance link id"), normalizeElectionYear(input.electionYear), JSON.stringify(keys)]
  );
...
async function deleteStaleOutsideGroups(input: {
  ...
  linkId: string;
  electionYear: number;
  groups: readonly WashingtonFinanceOutsideGroupInput[];
}): Promise<void> {
  const keys = input.groups.map((group) => ({
    sponsor_id: requireNonEmpty(group.sponsorId, "Washington outside group sponsor id"),
    support_oppose: group.supportOppose,
  }));

  await input.db.query(
    `
      DELETE FROM public.wa_candidate_finance_outside_groups
      WHERE link_id = $1::uuid
        AND election_year = $2
        AND NOT EXISTS (
          SELECT 1
          ...
          WHERE keep.sponsor_id = wa_candidate_finance_outside_groups.sponsor_id
            AND keep.support_oppose = wa_candidate_finance_outside_groups.support_oppose
        )
    `,
    [requireNonEmpty(input.linkId, "Washington finance link id"), normalizeElectionYear(input.electionYear), JSON.stringify(keys)]
  );
...
export async function replaceWashingtonCandidateFinanceSnapshot(
  input: WashingtonFinanceSnapshotInput
): Promise<WashingtonFinanceSnapshotWriteResult> {
  ...
  return await withWashingtonFinanceTransaction(input.db, async (db) => {
    const { linkId } = await upsertWashingtonFinanceLink({ db, link: input.link });
    if (input.summary) {
