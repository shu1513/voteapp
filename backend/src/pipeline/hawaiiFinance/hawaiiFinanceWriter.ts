...
export async function upsertHawaiiFinanceLink(input: {
  ...
  const result = await input.db.query<ManualProtectedLinkRow>(
    `
      ...
      DO UPDATE SET
        ...
        ${manualProtectedLinkAssignments("hi_candidate_finance_links")},
  ...
  assertLinkWriteNotBlocked("Hawaii", result.rows[0], input.link.linkSource ?? "manual", input.link.electionYear);
