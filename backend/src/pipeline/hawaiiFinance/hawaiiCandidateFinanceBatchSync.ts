...
type HawaiiCandidateFinanceDueQueryRow = {
  candidate_id: string;
  election_id: string;
...
function mapDueRow(row: HawaiiCandidateFinanceDueQueryRow): HawaiiCandidateFinanceDueRow {
  return {
    ...
    electionPeriod: row.election_period,
    sourceUrl: row.source_url,
    lastSyncedAt: row.last_synced_at,
  };
}

export async function listDueHawaiiCandidateFinanceSyncRows(
  db: Queryable,
  input: {
    now: Date;
    staleAfterDays: number;
...
): Promise<{ rows: HawaiiCandidateFinanceDueRow[]; totalDueRows: number }> {
  const result = await db.query<HawaiiCandidateFinanceDueQueryRow>(
    `
      WITH due AS (
        SELECT
