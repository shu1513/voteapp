// The cfrs.wvsos.gov TLS endpoint serves its leaf certificate without the
// issuing intermediate (verified 2026-08-26: `Verify return code: 21`), so
// default Node trust fails with UNABLE_TO_VERIFY_LEAF_SIGNATURE. This is the
// missing public intermediate, fetched from the leaf certificate's Authority
// Information Access URL:
//   http://crt.sectigo.com/EntrustDVTLSIssuingRSACA2.crt
// Subject: C=CA, O=Entrust Limited, CN=Entrust DV TLS Issuing RSA CA 2
// Issuer:  C=GB, O=Sectigo Limited, CN=Sectigo Public Server Authentication Root R46
// Valid:   2024-12-11 -> 2027-12-10
// SHA-256: EC:37:DF:84:99:95:A5:30:89:C9:8B:D1:2E:CA:C7:74:06:A2:B4:14:78:77:9E:5C:B8:2C:5F:16:6E:7D:F8:3C
// It is appended to Node's bundled root store for this exact host only —
// verification stays enabled; if the state fixes its chain the fallback
// simply stops being used.
export const WEST_VIRGINIA_CFRS_INTERMEDIATE_CA_PEM = `-----BEGIN CERTIFICATE-----

+CgUsrUKgzo4
-----END CERTIFICATE-----
`;
