// South Carolina finance due list — link-gated staleness query (Missouri
// shape, Delaware SQL): active links whose summary is missing or older than
// staleAfterDays, for eligible offices with a general election inside the
// lookback/lookahead window. Never-synced links sort first. Bespoke SQL
// rather than the shared builder because the sync needs election_date (to
// derive the statutory cycle dates) and the general-stage filter.
...
type DbRow = {
  ...
  link_source: SouthCarolinaFinanceLinkSource;
...
export async function listDueSouthCarolinaCandidateFinanceSyncRows(
  db: Queryable,
  input: StandardStateFinanceDueListInput
): Promise<StandardStateFinanceDueListResult<SouthCarolinaCandidateFinanceDueRow>> {
  const result = await db.query<DbRow>(
    `WITH due AS (
      SELECT link.candidate_id::text candidate_id, link.election_id::text election_id,
        ...
        link.candidate_filer_id, link.candidate_filer_name, link.link_source, link.source_url, summary.last_synced_at::text last_synced_at,
        COUNT(*) OVER () total_due_rows
      FROM public.sc_candidate_finance_links link
      ...
      LEFT JOIN public.sc_candidate_finance_summaries summary ON summary.link_id=link.id AND summary.election_year=link.election_year
      WHERE link.link_status='active' AND candidate.deleted_at IS NULL AND district_row.state='SC' AND election.race_type='office'
  ...
  return {
    rows: result.rows.map((row) => ({
      ...
      candidateFilerId: parseStoredFilerId(row.candidate_filer_id),
      filerName: row.candidate_filer_name,
