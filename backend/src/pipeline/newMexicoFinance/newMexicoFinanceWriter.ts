...
export type NewMexicoFinanceLinkSource = "manual" | "cfis_bulk";
export type NewMexicoFinanceDirectCategoryType =
  | "occupation"
  | "industry"
  | "contribution_size"
  | "contributor_source_type";
export type NewMexicoFinanceOutsideCategoryType = "donor" | "occupation" | "industry";
export type NewMexicoFinanceSupportOppose = "support" | "oppose";

export type NewMexicoFinanceLinkInput = {
  candidateId: string;
  electionId: string;
  electionYear: number;
  ...
  linkStatus?: NewMexicoFinanceLinkStatus;
  linkSource?: NewMexicoFinanceLinkSource;
  sourceUrl?: string | null;
  lastVerifiedAt?: Date | null;
};

export type NewMexicoFinanceSummaryInput = {
...
export type NewMexicoFinanceDirectBreakdownInput = {
  categoryType: NewMexicoFinanceDirectCategoryType;
  categoryName: string;
  amount: number;
...
export type NewMexicoFinanceOutsideGroupInput = {
  committeeId: string;
  committeeName: string;
  supportOppose: NewMexicoFinanceSupportOppose;
  amount: number;
  sourceUrl?: string | null;
};

export type NewMexicoFinanceOutsideGroupBreakdownInput = {
  committeeId: string;
  supportOppose: NewMexicoFinanceSupportOppose;
  categoryType: NewMexicoFinanceOutsideCategoryType;
  categoryName: string;
  amount: number;
  contributorCount?: number | null;
  sourceUrl?: string | null;
...
export type NewMexicoFinanceSnapshotInput = {
  db: Queryable;
  link: NewMexicoFinanceLinkInput;
  syncedAt?: Date;
  summary?: NewMexicoFinanceSummaryInput;
...
function normalizeNullableDate(value: Date | null | undefined): string | null {
  ...
  if (Number.isNaN(value.getTime())) {
    throw new Error("Invalid New Mexico finance timestamp");
  }
...
function normalizeNullableCount(value: number | null | undefined): number | null {
  ...
  if (!Number.isInteger(value) || value < 0) {
    throw new Error("New Mexico finance contributor count must be a nonnegative integer");
  }
  return value;
}
...
function validateNewMexicoFinanceLinkInput(link: NewMexicoFinanceLinkInput): void {
  ...
  requireNonEmpty(link.candidateNameNormalized, "New Mexico finance candidate name");
  requireNonEmpty(link.officeName, "New Mexico finance office name");
  requireNonEmpty(link.committeeId, "New Mexico committee id");
  requireNonEmpty(link.committeeName, "New Mexico committee name");
...
export async function upsertNewMexicoFinanceLink(input: {
  db: Queryable;
  link: NewMexicoFinanceLinkInput;
}): Promise<{ linkId: string }> {
  validateNewMexicoFinanceLinkInput(input.link);

  const result = await input.db.query<ManualProtectedLinkRow>(
    `
      INSERT INTO public.nm_candidate_finance_links (
        candidate_id,
        election_id,
        election_year,
      ...
      DO UPDATE SET
        ...
        ${manualProtectedLinkAssignments("nm_candidate_finance_links")},
    ...
    [
      ...
      requireNonEmpty(input.link.candidateNameNormalized, "New Mexico finance candidate name"),
      requireNonEmpty(input.link.officeName, "New Mexico finance office name"),
      normalizeOptionalText(input.link.district),
      requireNonEmpty(input.link.committeeId, "New Mexico committee id"),
      requireNonEmpty(input.link.committeeName, "New Mexico committee name"),
  ...
  assertLinkWriteNotBlocked("New Mexico", result.rows[0], input.link.linkSource ?? "manual", input.link.electionYear);
  const linkId = result.rows[0]?.id;
  if (!linkId) {
    throw new Error("New Mexico finance link upsert did not return an id");
  }
  return { linkId };
}
...
async function upsertSummary(input: {
  ...
  await input.db.query(
    `
      INSERT INTO public.nm_candidate_finance_summaries (
        link_id,
        election_year,
        total_receipts,
      ...
      DO UPDATE SET
        ...
        direct_contribution_total = COALESCE(EXCLUDED.direct_contribution_total, nm_candidate_finance_summaries.direct_contribution_total),
        total_disbursements = COALESCE(EXCLUDED.total_disbursements, nm_candidate_finance_summaries.total_disbursements),
        outside_support_total = COALESCE(EXCLUDED.outside_support_total, nm_candidate_finance_summaries.outside_support_total),
        outside_oppose_total = COALESCE(EXCLUDED.outside_oppose_total, nm_candidate_finance_summaries.outside_oppose_total),
        source_url = COALESCE(EXCLUDED.source_url, nm_candidate_finance_summaries.source_url),
        last_synced_at = EXCLUDED.last_synced_at
    `,
    [
      requireNonEmpty(input.linkId, "New Mexico finance link id"),
...
async function upsertDirectBreakdown(input: {
  ...
  linkId: string;
  electionYear: number;
  breakdown: NewMexicoFinanceDirectBreakdownInput;
  syncedAt: Date;
}): Promise<void> {
  await input.db.query(
    `
      INSERT INTO public.nm_candidate_finance_direct_breakdowns (
        link_id,
        election_year,
        category_type,
        category_name,
    ...
    [
      ...
      requireNonEmpty(input.breakdown.categoryName, "New Mexico finance direct breakdown category"),
...
async function upsertOutsideGroup(input: {
  ...
  linkId: string;
  electionYear: number;
  group: NewMexicoFinanceOutsideGroupInput;
  syncedAt: Date;
}): Promise<void> {
  await input.db.query(
    `
      INSERT INTO public.nm_candidate_finance_outside_groups (
        link_id,
        election_year,
        committee_id,
        committee_name,
    ...
    [
      requireNonEmpty(input.linkId, "New Mexico finance link id"),
      normalizeElectionYear(input.electionYear),
      requireNonEmpty(input.group.committeeId, "New Mexico outside group committee id"),
      requireNonEmpty(input.group.committeeName, "New Mexico outside group committee name"),
...
async function upsertOutsideGroupBreakdown(input: {
  ...
  await input.db.query(
    `
      INSERT INTO public.nm_candidate_finance_outside_group_breakdowns (
        link_id,
        election_year,
        committee_id,
        support_oppose,
    ...
    [
      requireNonEmpty(input.linkId, "New Mexico finance link id"),
      normalizeElectionYear(input.electionYear),
      requireNonEmpty(input.breakdown.committeeId, "New Mexico outside breakdown committee id"),
      input.breakdown.supportOppose,
      input.breakdown.categoryType,
      requireNonEmpty(input.breakdown.categoryName, "New Mexico outside breakdown category"),
...
async function deleteStaleDirectBreakdowns(input: {
  ...
  linkId: string;
  electionYear: number;
  breakdowns: readonly NewMexicoFinanceDirectBreakdownInput[];
}): Promise<void> {
  const keys = input.breakdowns.map((breakdown) => ({
    category_type: breakdown.categoryType,
    category_name: requireNonEmpty(breakdown.categoryName, "New Mexico finance direct breakdown category"),
  }));

  await input.db.query(
    `
      DELETE FROM public.nm_candidate_finance_direct_breakdowns
      WHERE link_id = $1::uuid
        ...
        AND NOT EXISTS (
          ...
          WHERE keep.category_type = nm_candidate_finance_direct_breakdowns.category_type
            AND keep.category_name = nm_candidate_finance_direct_breakdowns.category_name
        )
    `,
    [requireNonEmpty(input.linkId, "New Mexico finance link id"), normalizeElectionYear(input.electionYear), JSON.stringify(keys)]
  );
}
...
async function deleteStaleOutsideGroupBreakdowns(input: {
  ...
  const keys = input.breakdowns.map((breakdown) => ({
    committee_id: requireNonEmpty(breakdown.committeeId, "New Mexico outside breakdown committee id"),
    support_oppose: breakdown.supportOppose,
    category_type: breakdown.categoryType,
    category_name: requireNonEmpty(breakdown.categoryName, "New Mexico outside breakdown category"),
  }));

  await input.db.query(
    `
      DELETE FROM public.nm_candidate_finance_outside_group_breakdowns
      WHERE link_id = $1::uuid
        AND election_year = $2
        AND NOT EXISTS (
          SELECT 1
          ...
          WHERE keep.committee_id = nm_candidate_finance_outside_group_breakdowns.committee_id
            AND keep.support_oppose = nm_candidate_finance_outside_group_breakdowns.support_oppose
            AND keep.category_type = nm_candidate_finance_outside_group_breakdowns.category_type
            AND keep.category_name = nm_candidate_finance_outside_group_breakdowns.category_name
        )
    `,
    [requireNonEmpty(input.linkId, "New Mexico finance link id"), normalizeElectionYear(input.electionYear), JSON.stringify(keys)]
  );
...
async function deleteStaleOutsideGroups(input: {
  ...
  linkId: string;
  electionYear: number;
  groups: readonly NewMexicoFinanceOutsideGroupInput[];
}): Promise<void> {
  const keys = input.groups.map((group) => ({
    committee_id: requireNonEmpty(group.committeeId, "New Mexico outside group committee id"),
    support_oppose: group.supportOppose,
  }));

  await input.db.query(
    `
      DELETE FROM public.nm_candidate_finance_outside_groups
      WHERE link_id = $1::uuid
        AND election_year = $2
        AND NOT EXISTS (
          SELECT 1
          ...
          WHERE keep.committee_id = nm_candidate_finance_outside_groups.committee_id
            AND keep.support_oppose = nm_candidate_finance_outside_groups.support_oppose
        )
    `,
    [requireNonEmpty(input.linkId, "New Mexico finance link id"), normalizeElectionYear(input.electionYear), JSON.stringify(keys)]
  );
...
export async function replaceNewMexicoCandidateFinanceSnapshot(
  input: NewMexicoFinanceSnapshotInput
): Promise<NewMexicoFinanceSnapshotWriteResult> {
  ...
  return await withNewMexicoFinanceTransaction(input.db, async (db) => {
    const { linkId } = await upsertNewMexicoFinanceLink({ db, link: input.link });
    if (input.summary) {
