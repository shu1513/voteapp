...
export const listDueLouisianaCandidateFinanceSyncRows = createStandardStateFinanceDueListQuery({
  state: "LA",
  tables: {
    links: "la_candidate_finance_links",
    summaries: "la_candidate_finance_summaries",
  },
  // The Louisiana office-keys constant is a Set (most states export arrays);
  // spread once here — same values and order the bespoke per-call spread produced.
  eligibleOfficeKeys: [...LOUISIANA_FINANCE_ELIGIBLE_OFFICE_KEYS],
  linkColumns: ["filer_number", "filer_name"],
  mapRow: (row): LouisianaCandidateFinanceDueRow => ({
    ...
    filerNumber: row.filer_number as string,
