...
export type VirginiaFinanceLinkStatus = "active" | "inactive";
export type VirginiaFinanceLinkSource = "manual" | "cfreports_search" | "cfreports_xml";
export type VirginiaFinanceDirectCategoryType = "occupation" | "contribution_size";

export type VirginiaFinanceLinkInput = {
  candidateId: string;
  electionId: string;
  electionYear: number;
  ...
  linkStatus?: VirginiaFinanceLinkStatus;
  linkSource?: VirginiaFinanceLinkSource;
  sourceUrl?: string | null;
  lastVerifiedAt?: Date | null;
};

export type VirginiaFinanceSummaryInput = {
  totalReceipts?: number | null;
  directContributionTotal?: number | null;
  sourceUrl?: string | null;
};

export type VirginiaFinanceDirectBreakdownInput = {
  categoryType: VirginiaFinanceDirectCategoryType;
  categoryName: string;
  amount: number;
...
export type VirginiaFinanceSnapshotInput = {
  db: Queryable;
  link: VirginiaFinanceLinkInput;
  syncedAt?: Date;
  summary?: VirginiaFinanceSummaryInput;
  directBreakdowns?: readonly VirginiaFinanceDirectBreakdownInput[];
};

export type VirginiaFinanceSnapshotWriteResult = {
  linkId: string;
...
function normalizeNullableDate(value: Date | null | undefined): string | null {
  ...
  if (Number.isNaN(value.getTime())) {
    throw new Error("Invalid Virginia finance timestamp");
  }
...
function normalizeNullableCount(value: number | null | undefined): number | null {
  ...
  if (!Number.isInteger(value) || value < 0) {
    throw new Error("Virginia finance contributor count must be a nonnegative integer");
  }
  return value;
}
...
async function withVirginiaFinanceTransaction<T>(db: Queryable, work: (tx: Queryable) => Promise<T>): Promise<T> {
  assertSnapshotDbIsNotPoolClient("Virginia", db);
...
export async function upsertVirginiaFinanceLink(input: {
  db: Queryable;
  link: VirginiaFinanceLinkInput;
}): Promise<{ linkId: string }> {
  const result = await input.db.query<ManualProtectedLinkRow>(
    `
      INSERT INTO public.va_candidate_finance_links (
        candidate_id,
        election_id,
        election_year,
      ...
      DO UPDATE SET
        ...
        ${manualProtectedLinkAssignments("va_candidate_finance_links")},
    ...
    [
      ...
      requireNonEmpty(input.link.candidateNameNormalized, "Virginia finance candidate name"),
      requireNonEmpty(input.link.officeName, "Virginia finance office name"),
      normalizeOptionalText(input.link.district),
      requireNonEmpty(input.link.committeeId, "Virginia committee id"),
      normalizeOptionalText(input.link.committeeCode),
      requireNonEmpty(input.link.committeeName, "Virginia committee name"),
...
async function upsertSummary(input: {
  ...
  await input.db.query(
    `
      INSERT INTO public.va_candidate_finance_summaries (
        link_id,
        election_year,
        total_receipts,
      ...
      DO UPDATE SET
        ...
        source_url = COALESCE(EXCLUDED.source_url, va_candidate_finance_summaries.source_url),
        last_synced_at = EXCLUDED.last_synced_at
    `,
    [
      requireNonEmpty(input.linkId, "Virginia finance link id"),
...
async function upsertDirectBreakdown(input: {
  ...
  linkId: string;
  electionYear: number;
  breakdown: VirginiaFinanceDirectBreakdownInput;
  syncedAt: Date;
}): Promise<void> {
  await input.db.query(
    `
      INSERT INTO public.va_candidate_finance_direct_breakdowns (
        link_id,
        election_year,
        category_type,
        category_name,
    ...
    [
      ...
      requireNonEmpty(input.breakdown.categoryName, "Virginia finance direct breakdown category"),
...
async function deleteStaleDirectBreakdowns(input: {
  ...
  linkId: string;
  electionYear: number;
  breakdowns: readonly VirginiaFinanceDirectBreakdownInput[];
}): Promise<void> {
  const keys = input.breakdowns.map((breakdown) => ({
    category_type: breakdown.categoryType,
    category_name: requireNonEmpty(breakdown.categoryName, "Virginia finance direct breakdown category"),
  }));
  await input.db.query(
    `
      DELETE FROM public.va_candidate_finance_direct_breakdowns
      WHERE link_id = $1::uuid
        ...
        AND NOT EXISTS (
          ...
          WHERE keep.category_type = va_candidate_finance_direct_breakdowns.category_type
            AND keep.category_name = va_candidate_finance_direct_breakdowns.category_name
        )
    `,
    [requireNonEmpty(input.linkId, "Virginia finance link id"), normalizeElectionYear(input.electionYear), JSON.stringify(keys)]
  );
}

export async function replaceVirginiaCandidateFinanceSnapshot(
  input: VirginiaFinanceSnapshotInput
): Promise<VirginiaFinanceSnapshotWriteResult> {
  ...
  return await withVirginiaFinanceTransaction(input.db, async (db) => {
    const { linkId } = await upsertVirginiaFinanceLink({ db, link: input.link });
    if (input.summary) {
