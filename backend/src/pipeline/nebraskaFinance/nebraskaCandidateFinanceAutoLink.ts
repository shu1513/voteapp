...
import { upsertNebraskaFinanceLink } from "./nebraskaFinanceWriter.js";
import type { NebraskaNadcContributionRow } from "./nebraskaNadcArtifactReader.js";

type Queryable = Pick<Pool | PoolClient, "query">;

export type NebraskaFinanceAutoLinkCandidateElection = {
  candidateId: string;
  electionId: string;
...
export type NebraskaFinanceAutoLinkResult =
  | {
      candidateId: string;
      electionId: string;
      status: NebraskaCandidateCommitteeResolution["status"] | "linked";
      committeeId?: string;
      reason?: string;
    }
  | {
...
function mapCandidateElectionRow(row: CandidateElectionQueryRow): NebraskaFinanceAutoLinkCandidateElection {
  return {
...
export async function listNebraskaCandidateElectionsMissingFinanceLinks(
  db: Queryable,
  input: {
    now: Date;
...
): Promise<NebraskaFinanceAutoLinkCandidateElection[]> {
  const result = await db.query<CandidateElectionQueryRow>(
    `
      SELECT
        ...
        CASE
          WHEN district.district_type = 'state_upper' THEN
            NULLIF(
              regexp_replace(
...
export async function autoLinkNebraskaCandidateFinanceForCandidateElection(input: {
  db: Queryable;
  candidateElection: NebraskaFinanceAutoLinkCandidateElection;
  contributionRows: readonly NebraskaNadcContributionRow[];
  sourceUrl: string | null;
  now: Date;
}): Promise<NebraskaFinanceAutoLinkResult> {
  ...
  await upsertNebraskaFinanceLink({
    db: input.db,
    link: {
      candidateId: input.candidateElection.candidateId,
      electionId: input.candidateElection.electionId,
...
export async function autoLinkMissingNebraskaCandidateFinanceLinks(input: {
  db: Queryable;
  now: Date;
  maxCandidates: number;
  electionLookbackDays: number;
  electionLookaheadDays: number;
  contributionRowsByYear: ReadonlyMap<number, readonly NebraskaNadcContributionRow[]>;
  sourceUrlByYear?: ReadonlyMap<number, string>;
  candidateElections?: readonly NebraskaFinanceAutoLinkCandidateElection[];
}): Promise<NebraskaFinanceAutoLinkResult[]> {
  const candidates =
    input.candidateElections ??
    (await listNebraskaCandidateElectionsMissingFinanceLinks(input.db, {
      now: input.now,
  ...
  const results: NebraskaFinanceAutoLinkResult[] = [];
  for (const candidateElection of candidates) {
    try {
      results.push(
        await autoLinkNebraskaCandidateFinanceForCandidateElection({
          db: input.db,
          candidateElection,
      ...
      console.warn("Nebraska finance auto-link failed for candidate election; continuing:", {
