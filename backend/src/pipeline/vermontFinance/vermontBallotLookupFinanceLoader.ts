...
export type VermontBallotLookupCandidateRow = {
  election_id: string;
  candidate_id: string;
};

export type VermontBallotLookupElectionRow = {
  election_id: string;
  state: string;
...
type VermontFinanceSummaryRequest = {
  candidate_id: string;
  election_id: string;
};

type VermontFinanceSummaryRow = {
  candidate_id: string;
  election_id: string;
...
type VermontFinanceDirectBreakdownRow = {
  candidate_id: string;
  election_id: string;
...
type VermontFinanceOutsideGroupRow = {
  candidate_id: string;
  election_id: string;
  committee_id: string;
...
type VermontFinanceOutsideIndustryRow = {
  candidate_id: string;
  election_id: string;
  support_oppose: "support" | "oppose";
  category_name: string;
...
type VermontFinanceOutsideDonorEvidenceRow = {
  candidate_id: string;
  election_id: string;
...
function buildVermontFinanceSummaryRequests(
...
): VermontFinanceSummaryRequest[] {
  const electionIds = new Set(
    electionRows
      .filter(
        (row) =>
          row.state.trim().toUpperCase() === "VT" &&
  ...
  const requests = new Map<string, VermontFinanceSummaryRequest>();
...
export async function loadVermontCandidateFinanceSummariesByCandidateElection(
  ...
  const summaryResult = await db.query<VermontFinanceSummaryRow>(
    `
      WITH requested AS (
        SELECT
      ...
      SELECT
        ...
        CASE
          WHEN count(DISTINCT link.filer_registration_guid) = 1 THEN min(link.filer_registration_guid)
          ELSE NULL
        END AS committee_id,
      ...
      FROM requested
      JOIN public.vt_candidate_finance_links AS link
        ON link.candidate_id = requested.candidate_id
       AND link.election_id = requested.election_id
       AND link.link_status = 'active'
      JOIN public.vt_candidate_finance_summaries AS summary
  ...
  const directBreakdownResult = await db.query<VermontFinanceDirectBreakdownRow>(
    `
      WITH selected AS (
        SELECT
          candidate_id::uuid AS candidate_id,
      ...
      grouped AS (
        ...
        JOIN public.vt_candidate_finance_links AS link
         ...
         AND link.link_status = 'active'
        JOIN public.vt_candidate_finance_direct_breakdowns AS breakdown
  ...
  const outsideGroupResult = await db.query<VermontFinanceOutsideGroupRow>(
    `
      WITH selected AS (
        SELECT
          candidate_id::uuid AS candidate_id,
      ...
      grouped AS (
        SELECT
          selected.candidate_id::text AS candidate_id,
          selected.election_id::text AS election_id,
          outside_group.filer_registration_guid AS committee_id,
  ...
  const outsideIndustryResult = await db.query<VermontFinanceOutsideIndustryRow>(
    `
      WITH selected AS (
        SELECT
          candidate_id::uuid AS candidate_id,
      ...
      per_group AS (
        SELECT
          selected.candidate_id::text AS candidate_id,
          selected.election_id::text AS election_id,
          breakdown.filer_registration_guid,
        ...
        JOIN public.vt_candidate_finance_links AS link
         ...
         AND link.election_id = selected.election_id
         AND link.link_status = 'active'
        JOIN public.vt_candidate_finance_outside_group_breakdowns AS breakdown
          ON breakdown.link_id = link.id
         AND breakdown.election_year = link.election_year
  ...
  const outsideDonorEvidenceResult = await db.query<VermontFinanceOutsideDonorEvidenceRow>(
    `
      WITH selected AS (
        SELECT
          candidate_id::uuid AS candidate_id,
      ...
      top_industries_per_group AS (
        SELECT
          selected.candidate_id::text AS candidate_id,
          selected.election_id::text AS election_id,
          industry.filer_registration_guid AS committee_id,
        ...
        JOIN public.vt_candidate_finance_links AS link
         ...
         AND link.election_id = selected.election_id
         AND link.link_status = 'active'
        JOIN public.vt_candidate_finance_outside_group_breakdowns AS industry
        ...
        GROUP BY selected.candidate_id, selected.election_id, industry.filer_registration_guid, industry.category_name
      ),
      ...
      evidence AS (
        SELECT
          ...
          breakdown.filer_registration_guid AS committee_id,
          COALESCE(outside_group.filer_name, breakdown.filer_registration_guid) AS committee_name,
          breakdown.support_oppose,
          ...
          row_number() OVER (
            ...
            ORDER BY breakdown.amount DESC, breakdown.category_name ASC, breakdown.filer_registration_guid ASC
          ) AS rn
        FROM selected
        JOIN top_industries
        ...
        LEFT JOIN public.vt_candidate_finance_outside_groups AS outside_group
          ON outside_group.link_id = breakdown.link_id
         AND outside_group.election_year = breakdown.election_year
         AND outside_group.filer_registration_guid = breakdown.filer_registration_guid
  ...
  for (const row of directBreakdownResult.rows) {
    ...
    addFinanceBreakdown(
      ...
      row.candidate_id,
      row.election_id,
      mapFinanceBreakdown(row, summary?.source_url ?? GENERIC_VERMONT_CFD_SOURCE_URL)
    );
  }
  ...
  for (const row of outsideGroupResult.rows) {
    ...
    list.push({
      ...
      amount: parseFinanceAmount(row.amount) ?? 0,
      source_url: firstNonEmptySourceUrl(row.source_url, GENERIC_VERMONT_CFD_SOURCE_URL),
    });
    map.set(key, list);
  ...
  for (const row of outsideIndustryResult.rows) {
    ...
    addFinanceBreakdown(
      ...
      row.candidate_id,
      row.election_id,
      mapFinanceBreakdown(row, GENERIC_VERMONT_CFD_SOURCE_URL)
    );
  }
