...
export type HoustonFinanceLinkStatus = "active" | "inactive";
export type HoustonFinanceLinkSource = "manual" | "houston_reports";
export type HoustonFinanceDirectCategoryType = "occupation" | "contribution_size";
export type HoustonFinanceOutsideCategoryType = "donor" | "industry";
export type HoustonFinanceSupportOppose = "support" | "oppose";

export type HoustonFinanceLinkInput = {
  candidateId: string;
  electionId: string;
  electionYear: number;
  ...
  linkStatus?: HoustonFinanceLinkStatus;
  linkSource?: HoustonFinanceLinkSource;
  sourceUrl?: string | null;
  lastVerifiedAt?: Date | null;
};

export type HoustonFinanceSummaryInput = {
...
export type HoustonFinanceDirectBreakdownInput = {
  categoryType: HoustonFinanceDirectCategoryType;
  categoryName: string;
  amount: number;
...
export type HoustonFinanceOutsideGroupInput = {
  committeeId: string;
  committeeName: string;
  supportOppose: HoustonFinanceSupportOppose;
  amount: number;
  sourceUrl?: string | null;
};

export type HoustonFinanceOutsideGroupBreakdownInput = {
  committeeId: string;
  supportOppose: HoustonFinanceSupportOppose;
  categoryType: HoustonFinanceOutsideCategoryType;
  categoryName: string;
  amount: number;
  contributorCount?: number | null;
  sourceUrl?: string | null;
...
export type HoustonFinanceSnapshotWriteResult = {
  linkId: string;
...
function normalizeElectionYear(value: number): number {
  if (!Number.isInteger(value) || value < 2014 || value > 2100) {
    throw new Error(`Invalid Houston finance election year: ${value}`);
  }
  return value;
}
...
function normalizeNullableDate(value: Date | null | undefined): string | null {
  ...
  if (Number.isNaN(value.getTime())) {
    throw new Error("Invalid Houston finance timestamp");
  }
...
function normalizeNullableCount(value: number | null | undefined): number | null {
  ...
  if (!Number.isInteger(value) || value < 0) {
    throw new Error("Houston finance contributor count must be a nonnegative integer");
  }
  return value;
}
...
function validateHoustonFinanceLinkInput(link: HoustonFinanceLinkInput): void {
  ...
  requireNonEmpty(link.candidateNameNormalized, "Houston finance candidate name");
  requireNonEmpty(link.officeName, "Houston finance office name");
  requireNonEmpty(link.committeeId, "Houston committee id");
  requireNonEmpty(link.committeeName, "Houston committee name");
  normalizeNullableDate(link.lastVerifiedAt);
}

async function withHoustonFinanceTransaction<T>(db: Queryable, work: (tx: Queryable) => Promise<T>): Promise<T> {
  if (!canOpenTransaction(db)) {
    if (isClientLikeQueryable(db)) {
      throw new Error("Houston finance snapshot writes must receive a Pool, not a PoolClient");
    }
    try {
...
export async function upsertHoustonFinanceLink(input: {
  ...
  validateHoustonFinanceLinkInput(input.link);

  const result = await input.db.query<{ id: string }>(
    `
      INSERT INTO public.hou_candidate_finance_links (
        candidate_id,
        election_id,
        election_year,
    ...
    [
      ...
      requireNonEmpty(input.link.candidateNameNormalized, "Houston finance candidate name"),
      requireNonEmpty(input.link.officeName, "Houston finance office name"),
      normalizeOptionalText(input.link.district),
      requireNonEmpty(input.link.committeeId, "Houston committee id"),
      requireNonEmpty(input.link.committeeName, "Houston committee name"),
  ...
  if (!linkId) {
    throw new Error("Houston finance link upsert did not return an id");
  }
  return { linkId };
}
...
async function upsertSummary(input: {
  ...
  await input.db.query(
    `
      INSERT INTO public.hou_candidate_finance_summaries (
        link_id,
        election_year,
        total_receipts,
      ...
      DO UPDATE SET
        total_receipts = COALESCE(EXCLUDED.total_receipts, hou_candidate_finance_summaries.total_receipts),
        direct_contribution_total = COALESCE(EXCLUDED.direct_contribution_total, hou_candidate_finance_summaries.direct_contribution_total),
        total_disbursements = COALESCE(EXCLUDED.total_disbursements, hou_candidate_finance_summaries.total_disbursements),
        cash_on_hand = COALESCE(EXCLUDED.cash_on_hand, hou_candidate_finance_summaries.cash_on_hand),
        outside_support_total = COALESCE(EXCLUDED.outside_support_total, hou_candidate_finance_summaries.outside_support_total),
        outside_oppose_total = COALESCE(EXCLUDED.outside_oppose_total, hou_candidate_finance_summaries.outside_oppose_total),
        source_url = COALESCE(EXCLUDED.source_url, hou_candidate_finance_summaries.source_url),
        last_synced_at = EXCLUDED.last_synced_at
    `,
    [
      requireNonEmpty(input.linkId, "Houston finance link id"),
...
async function upsertDirectBreakdown(input: {
  ...
  linkId: string;
  electionYear: number;
  breakdown: HoustonFinanceDirectBreakdownInput;
  syncedAt: Date;
}): Promise<void> {
  await input.db.query(
    `
      INSERT INTO public.hou_candidate_finance_direct_breakdowns (
        link_id,
        election_year,
        category_type,
        category_name,
    ...
    [
      ...
      requireNonEmpty(input.breakdown.categoryName, "Houston finance direct breakdown category"),
...
async function upsertOutsideGroup(input: {
  ...
  linkId: string;
  electionYear: number;
  group: HoustonFinanceOutsideGroupInput;
  syncedAt: Date;
}): Promise<void> {
  await input.db.query(
    `
      INSERT INTO public.hou_candidate_finance_outside_groups (
        link_id,
        election_year,
        committee_id,
        committee_name,
    ...
    [
      requireNonEmpty(input.linkId, "Houston finance link id"),
      normalizeElectionYear(input.electionYear),
      requireNonEmpty(input.group.committeeId, "Houston outside group committee id"),
      requireNonEmpty(input.group.committeeName, "Houston outside group committee name"),
...
async function upsertOutsideGroupBreakdown(input: {
  ...
  linkId: string;
  electionYear: number;
  breakdown: HoustonFinanceOutsideGroupBreakdownInput;
  syncedAt: Date;
}): Promise<void> {
  await input.db.query(
    `
      INSERT INTO public.hou_candidate_finance_outside_group_breakdowns (
        link_id,
        election_year,
        committee_id,
        support_oppose,
    ...
    [
      requireNonEmpty(input.linkId, "Houston finance link id"),
      normalizeElectionYear(input.electionYear),
      requireNonEmpty(input.breakdown.committeeId, "Houston outside breakdown committee id"),
      input.breakdown.supportOppose,
      input.breakdown.categoryType,
      requireNonEmpty(input.breakdown.categoryName, "Houston outside breakdown category"),
...
async function deleteStaleDirectBreakdowns(input: {
  ...
  linkId: string;
  electionYear: number;
  breakdowns: readonly HoustonFinanceDirectBreakdownInput[];
}): Promise<void> {
  const keys = input.breakdowns.map((breakdown) => ({
    category_type: breakdown.categoryType,
    category_name: requireNonEmpty(breakdown.categoryName, "Houston finance direct breakdown category"),
  }));

  await input.db.query(
    `
      DELETE FROM public.hou_candidate_finance_direct_breakdowns
      WHERE link_id = $1::uuid
        ...
        AND NOT EXISTS (
          ...
          WHERE keep.category_type = hou_candidate_finance_direct_breakdowns.category_type
            AND keep.category_name = hou_candidate_finance_direct_breakdowns.category_name
        )
    `,
    [requireNonEmpty(input.linkId, "Houston finance link id"), normalizeElectionYear(input.electionYear), JSON.stringify(keys)]
  );
}
...
async function deleteStaleOutsideGroupBreakdowns(input: {
  ...
  const keys = input.breakdowns.map((breakdown) => ({
    committee_id: requireNonEmpty(breakdown.committeeId, "Houston outside breakdown committee id"),
    support_oppose: breakdown.supportOppose,
    category_type: breakdown.categoryType,
    category_name: requireNonEmpty(breakdown.categoryName, "Houston outside breakdown category"),
  }));

  await input.db.query(
    `
      DELETE FROM public.hou_candidate_finance_outside_group_breakdowns
      WHERE link_id = $1::uuid
        AND election_year = $2
        AND NOT EXISTS (
          SELECT 1
          ...
          WHERE keep.committee_id = hou_candidate_finance_outside_group_breakdowns.committee_id
            AND keep.support_oppose = hou_candidate_finance_outside_group_breakdowns.support_oppose
            AND keep.category_type = hou_candidate_finance_outside_group_breakdowns.category_type
            AND keep.category_name = hou_candidate_finance_outside_group_breakdowns.category_name
        )
    `,
    [requireNonEmpty(input.linkId, "Houston finance link id"), normalizeElectionYear(input.electionYear), JSON.stringify(keys)]
  );
...
async function deleteStaleOutsideGroups(input: {
  ...
  linkId: string;
  electionYear: number;
  groups: readonly HoustonFinanceOutsideGroupInput[];
}): Promise<void> {
  const keys = input.groups.map((group) => ({
    committee_id: requireNonEmpty(group.committeeId, "Houston outside group committee id"),
    support_oppose: group.supportOppose,
  }));

  await input.db.query(
    `
      DELETE FROM public.hou_candidate_finance_outside_groups
      WHERE link_id = $1::uuid
        AND election_year = $2
        AND NOT EXISTS (
          SELECT 1
          ...
          WHERE keep.committee_id = hou_candidate_finance_outside_groups.committee_id
            AND keep.support_oppose = hou_candidate_finance_outside_groups.support_oppose
        )
    `,
    [requireNonEmpty(input.linkId, "Houston finance link id"), normalizeElectionYear(input.electionYear), JSON.stringify(keys)]
  );
...
export async function replaceHoustonCandidateFinanceSnapshot(
  ...
  if (Number.isNaN(syncedAt.getTime())) {
    throw new Error("Invalid Houston finance sync timestamp");
  }
  validateHoustonFinanceLinkInput(input.link);
  const electionYear = normalizeElectionYear(input.link.electionYear);

  return await withHoustonFinanceTransaction(input.db, async (db) => {
    const { linkId } = await upsertHoustonFinanceLink({ db, link: input.link });
    if (input.summary) {
