    ...
    `
      WITH standard_candidate_fecs AS (
        ...
        WHERE c.deleted_at IS NULL
          ...
          AND e.election_date >= (($1::timestamptz)::date - make_interval(days => $4::int))
          AND e.election_date <= (($1::timestamptz)::date + make_interval(days => $5::int))
      ...
      presidential_candidate_fecs AS (
        ...
        WHERE c.deleted_at IS NULL
          ...
          AND cycle.election_year BETWEEN extract(year from ($1::timestamptz)::date)::int - 1 AND extract(year from ($1::timestamptz)::date)::int + 4
          AND COALESCE(
          ...
          ) >= (($1::timestamptz)::date - make_interval(days => $4::int))
