...
type AlaskaBallotLookupCandidateRow = {
  candidate_id: string;
  election_id: string;
};

type AlaskaBallotLookupElectionRow = {
  election_id: string;
  state: string;
...
type AlaskaFinanceSummaryRequest = {
  candidate_id: string;
  election_id: string;
};

type AlaskaFinanceSummaryRow = {
  candidate_id: string;
  election_id: string;
...
type AlaskaFinanceDirectBreakdownRow = {
  candidate_id: string;
  election_id: string;
...
type AlaskaFinanceOutsideGroupRow = {
  candidate_id: string;
  election_id: string;
  committee_id: string;
...
type AlaskaFinanceOutsideIndustryRow = {
  candidate_id: string;
  election_id: string;
  support_oppose: "support" | "oppose";
  category_name: string;
...
type AlaskaFinanceOutsideDonorEvidenceRow = {
  candidate_id: string;
  election_id: string;
...
function buildAlaskaFinanceSummaryRequests(
  candidateRows: readonly AlaskaBallotLookupCandidateRow[],
  electionRows: readonly AlaskaBallotLookupElectionRow[]
): AlaskaFinanceSummaryRequest[] {
  const electionIds = new Set(
    electionRows
      .filter(
        (row) =>
          row.state.trim().toUpperCase() === "AK" &&
          isAlaskaFinanceEligibleOffice({
  ...
  const requests = new Map<string, AlaskaFinanceSummaryRequest>();
...
export async function loadAlaskaCandidateFinanceSummariesByCandidateElection(
  ...
  const requests = buildAlaskaFinanceSummaryRequests(candidateRows, electionRows);
  if (requests.length === 0) {
    return new Map();
  }

  const summaryResult = await db.query<AlaskaFinanceSummaryRow>(
    `
      WITH requested AS (
        SELECT
      ...
      SELECT
        ...
        CASE
          WHEN count(DISTINCT link.candidate_filer_id) = 1 THEN min(link.candidate_filer_id)
          ELSE NULL
        END AS committee_id,
      ...
      FROM requested
      JOIN public.ak_candidate_finance_links AS link
        ON link.candidate_id = requested.candidate_id
       AND link.election_id = requested.election_id
       AND link.link_status = 'active'
      JOIN public.ak_candidate_finance_summaries AS summary
  ...
  const directBreakdownResult = await db.query<AlaskaFinanceDirectBreakdownRow>(
    `
      WITH selected AS (
        SELECT
          candidate_id::uuid AS candidate_id,
      ...
      grouped AS (
        ...
        JOIN public.ak_candidate_finance_links AS link
         ...
         AND link.link_status = 'active'
        JOIN public.ak_candidate_finance_direct_breakdowns AS breakdown
  ...
  const outsideGroupResult = await db.query<AlaskaFinanceOutsideGroupRow>(
    `
      WITH selected AS (
        SELECT
          candidate_id::uuid AS candidate_id,
      ...
      grouped AS (
        SELECT
          selected.candidate_id::text AS candidate_id,
          selected.election_id::text AS election_id,
          outside_group.outside_group_id AS committee_id,
          min(outside_group.outside_group_name) AS committee_name,
        ...
        JOIN public.ak_candidate_finance_links AS link
         ...
         AND link.election_id = selected.election_id
         AND link.link_status = 'active'
        JOIN public.ak_candidate_finance_outside_groups AS outside_group
          ON outside_group.link_id = link.id
         AND outside_group.election_year = link.election_year
        GROUP BY selected.candidate_id, selected.election_id, outside_group.outside_group_id, outside_group.support_oppose
      ),
      ranked AS (
        SELECT
          *,
  ...
  const outsideIndustryResult = await db.query<AlaskaFinanceOutsideIndustryRow>(
    `
      WITH selected AS (
        SELECT
          candidate_id::uuid AS candidate_id,
      ...
      per_group AS (
        ...
        JOIN public.ak_candidate_finance_links AS link
         ...
         AND link.election_id = selected.election_id
         AND link.link_status = 'active'
        JOIN public.ak_candidate_finance_outside_group_breakdowns AS breakdown
          ON breakdown.link_id = link.id
         AND breakdown.election_year = link.election_year
  ...
  const outsideDonorEvidenceResult = await db.query<AlaskaFinanceOutsideDonorEvidenceRow>(
    `
      WITH selected AS (
        SELECT
          candidate_id::uuid AS candidate_id,
      ...
      top_industries_per_group AS (
        SELECT
          selected.candidate_id::text AS candidate_id,
          selected.election_id::text AS election_id,
          industry.outside_group_id AS committee_id,
        ...
        JOIN public.ak_candidate_finance_links AS link
         ...
         AND link.election_id = selected.election_id
         AND link.link_status = 'active'
        JOIN public.ak_candidate_finance_outside_group_breakdowns AS industry
        ...
        GROUP BY selected.candidate_id, selected.election_id, industry.outside_group_id, industry.category_name
      ),
      ...
      evidence AS (
        SELECT
          ...
          breakdown.outside_group_id AS committee_id,
          COALESCE(outside_group.outside_group_name, breakdown.outside_group_id) AS committee_name,
          breakdown.support_oppose,
          ...
          row_number() OVER (
            ...
            ORDER BY breakdown.amount DESC, breakdown.category_name ASC, breakdown.outside_group_id ASC
          ) AS rn
        FROM selected
        JOIN top_industries
        ...
        LEFT JOIN public.ak_candidate_finance_outside_groups AS outside_group
          ON outside_group.link_id = breakdown.link_id
         AND outside_group.election_year = breakdown.election_year
         AND outside_group.outside_group_id = breakdown.outside_group_id
  ...
  for (const row of directBreakdownResult.rows) {
    ...
    addFinanceBreakdown(
      ...
      row.candidate_id,
      row.election_id,
      mapFinanceBreakdown(row, summary?.source_url ?? GENERIC_ALASKA_APOC_SOURCE_URL)
    );
  }
  ...
  for (const row of outsideGroupResult.rows) {
    ...
    list.push({
      ...
      amount: parseFinanceAmount(row.amount) ?? 0,
      source_url: firstNonEmptySourceUrl(row.source_url, GENERIC_ALASKA_APOC_SOURCE_URL),
    });
    map.set(key, list);
  ...
  for (const row of outsideIndustryResult.rows) {
    ...
    addFinanceBreakdown(
      ...
      row.candidate_id,
      row.election_id,
      mapFinanceBreakdown(row, GENERIC_ALASKA_APOC_SOURCE_URL)
    );
  }
