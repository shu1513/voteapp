...
export type MarylandFinanceLinkStatus = "active" | "inactive";
export type MarylandFinanceLinkSource = "manual" | "cfs_public_export";
export type MarylandFinanceDirectCategoryType = "occupation" | "contribution_size";
export type MarylandFinanceOutsideCategoryType = "donor" | "industry";
export type MarylandFinanceSupportOppose = "support" | "oppose";

export type MarylandFinanceLinkInput = {
  candidateId: string;
  electionId: string;
  electionYear: number;
  ...
  linkStatus?: MarylandFinanceLinkStatus;
  linkSource?: MarylandFinanceLinkSource;
  sourceUrl?: string | null;
  lastVerifiedAt?: Date | null;
};

export type MarylandFinanceSummaryInput = {
...
export type MarylandFinanceDirectBreakdownInput = {
  categoryType: MarylandFinanceDirectCategoryType;
  categoryName: string;
  amount: number;
...
export type MarylandFinanceOutsideGroupInput = {
  committeeId: string;
  committeeName: string;
...
export type MarylandFinanceOutsideGroupBreakdownInput = {
  committeeId: string;
  supportOppose: MarylandFinanceSupportOppose;
  categoryType: MarylandFinanceOutsideCategoryType;
  categoryName: string;
  amount: number;
  contributorCount?: number | null;
  sourceUrl?: string | null;
...
export type MarylandFinanceSnapshotWriteResult = {
  linkId: string;
...
function normalizeCommitteeId(value: string): string {
  return requireNonEmpty(value, "Maryland committee id").replace(/\s+/g, " ");
}
...
function normalizeNullableCount(value: number | null | undefined): number | null {
  ...
  if (!Number.isInteger(value) || value < 0) {
    throw new Error("Maryland finance contributor count must be a nonnegative integer");
  }
  return value;
}
...
async function withMarylandFinanceTransaction<T>(db: Queryable, work: (tx: Queryable) => Promise<T>): Promise<T> {
  if (!canOpenTransaction(db)) {
    if (isClientLikeQueryable(db)) {
      throw new Error("Maryland finance snapshot writes must receive a Pool, not a PoolClient");
    }
...
export async function upsertMarylandFinanceLink(input: {
  ...
  const result = await input.db.query<{ id: string }>(
    `
      INSERT INTO public.md_candidate_finance_links (
        candidate_id,
        election_id,
        election_year,
    ...
    [
      ...
      requireNonEmpty(input.link.candidateNameNormalized, "Maryland finance candidate name"),
      requireNonEmpty(input.link.officeName, "Maryland finance office name"),
      normalizeOptionalText(input.link.district),
      normalizeCommitteeId(input.link.committeeId),
      requireNonEmpty(input.link.committeeName, "Maryland committee name"),
  ...
  if (!linkId) {
    throw new Error("Maryland finance link upsert did not return an id");
  }
  return { linkId };
}
...
async function upsertSummary(input: {
  ...
  await input.db.query(
    `
      INSERT INTO public.md_candidate_finance_summaries (
        link_id,
        election_year,
        total_receipts,
    ...
    `,
    [
      requireNonEmpty(input.linkId, "Maryland finance link id"),
...
async function upsertDirectBreakdown(input: {
  ...
  linkId: string;
  electionYear: number;
  breakdown: MarylandFinanceDirectBreakdownInput;
  syncedAt: Date;
}): Promise<void> {
  await input.db.query(
    `
      INSERT INTO public.md_candidate_finance_direct_breakdowns (
        link_id,
        election_year,
        category_type,
        category_name,
    ...
    [
      ...
      requireNonEmpty(input.breakdown.categoryName, "Maryland finance direct breakdown category"),
...
async function upsertOutsideGroup(input: {
  ...
  await input.db.query(
    `
      INSERT INTO public.md_candidate_finance_outside_groups (
        link_id,
        election_year,
        committee_id,
        committee_name,
    ...
    [
      ...
      requireNonEmpty(input.group.committeeName, "Maryland outside group committee name"),
...
async function upsertOutsideGroupBreakdown(input: {
  ...
  await input.db.query(
    `
      INSERT INTO public.md_candidate_finance_outside_group_breakdowns (
        link_id,
        election_year,
        committee_id,
        support_oppose,
    ...
    [
      ...
      input.breakdown.categoryType,
      requireNonEmpty(input.breakdown.categoryName, "Maryland outside breakdown category"),
...
async function deleteStaleDirectBreakdowns(input: {
  ...
  linkId: string;
  electionYear: number;
  breakdowns: readonly MarylandFinanceDirectBreakdownInput[];
}): Promise<void> {
  const keys = input.breakdowns.map((breakdown) => ({
    category_type: breakdown.categoryType,
    category_name: requireNonEmpty(breakdown.categoryName, "Maryland finance direct breakdown category"),
  }));

  await input.db.query(
    `
      DELETE FROM public.md_candidate_finance_direct_breakdowns
      WHERE link_id = $1::uuid
        ...
        AND NOT EXISTS (
          ...
          WHERE keep.category_type = md_candidate_finance_direct_breakdowns.category_type
            AND keep.category_name = md_candidate_finance_direct_breakdowns.category_name
        )
    `,
    [requireNonEmpty(input.linkId, "Maryland finance link id"), normalizeElectionYear(input.electionYear), JSON.stringify(keys)]
  );
}
...
async function deleteStaleOutsideGroupBreakdowns(input: {
  ...
  const keys = input.breakdowns.map((breakdown) => ({
    ...
    category_type: breakdown.categoryType,
    category_name: requireNonEmpty(breakdown.categoryName, "Maryland outside breakdown category"),
  }));

  await input.db.query(
    `
      DELETE FROM public.md_candidate_finance_outside_group_breakdowns
      WHERE link_id = $1::uuid
        AND election_year = $2
        AND NOT EXISTS (
          SELECT 1
          ...
          WHERE keep.committee_id = md_candidate_finance_outside_group_breakdowns.committee_id
            AND keep.support_oppose = md_candidate_finance_outside_group_breakdowns.support_oppose
            AND keep.category_type = md_candidate_finance_outside_group_breakdowns.category_type
            AND keep.category_name = md_candidate_finance_outside_group_breakdowns.category_name
        )
    `,
    [requireNonEmpty(input.linkId, "Maryland finance link id"), normalizeElectionYear(input.electionYear), JSON.stringify(keys)]
  );
...
async function deleteStaleOutsideGroups(input: {
  ...
  linkId: string;
  electionYear: number;
  groups: readonly MarylandFinanceOutsideGroupInput[];
}): Promise<void> {
  ...
  await input.db.query(
    `
      DELETE FROM public.md_candidate_finance_outside_groups
      WHERE link_id = $1::uuid
        AND election_year = $2
        AND NOT EXISTS (
          SELECT 1
          ...
          WHERE keep.committee_id = md_candidate_finance_outside_groups.committee_id
            AND keep.support_oppose = md_candidate_finance_outside_groups.support_oppose
        )
    `,
    [requireNonEmpty(input.linkId, "Maryland finance link id"), normalizeElectionYear(input.electionYear), JSON.stringify(keys)]
  );
