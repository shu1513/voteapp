// Montana finance due list — link-gated staleness query (the Missouri/South
// Carolina shape): active links whose summary is missing or older than
...
// rather than the shared builder because Montana keeps the general-stage
// filter (we link FROM the Nov-2026 general roster only).
...
type DbRow = {
  ...
  link_source: MontanaFinanceLinkSource;
  source_url: string | null;
  last_synced_at: string | null;
  total_due_rows: string | number;
};

export async function listDueMontanaCandidateFinanceSyncRows(
  db: Queryable,
  input: StandardStateFinanceDueListInput
): Promise<StandardStateFinanceDueListResult<MontanaCandidateFinanceDueRow>> {
  const result = await db.query<DbRow>(
    `WITH due AS (
      ...
      FROM public.mt_candidate_finance_links link
      ...
      LEFT JOIN public.mt_candidate_finance_summaries summary ON summary.link_id=link.id AND summary.election_year=link.election_year
      WHERE link.link_status='active' AND candidate.deleted_at IS NULL AND district_row.state='MT' AND election.race_type='office'
  ...
  return {
    rows: result.rows.map((row) => ({
      ...
      committeeId: normalizeMontanaCersEntityId(row.committee_id),
