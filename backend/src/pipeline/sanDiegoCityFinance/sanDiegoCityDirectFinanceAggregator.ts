// Direct-campaign aggregation for a resolved San Diego city candidate
// committee (plan Phase 3) — a rename-only copy of the San José aggregator:
// the module is agency-agnostic (the Phase 0 probe ran the SJ implementation
// against the live San Diego workbooks and every gate reconciled cent-exact),
// and the plan forbids extracting a shared efileCal aggregator until a third
// tenant exists. Cycle formulas proven live 2026-08-10..12 on both cities:
//
...
export type SanDiegoCityDirectViolationType =
...
export type SanDiegoCityDirectViolation = {
...
export type SanDiegoCityDirectBreakdown = {
...
export type SanDiegoCityDirectFinanceAggregate = {
  ...
  breakdowns: SanDiegoCityDirectBreakdown[];
...
export function aggregateSanDiegoCityDirectFinance(input: {
  filerId: string;
...
}): SanDiegoCityDirectFinanceAggregate {
  const limit = input.maxBreakdownsPerCategory ?? 20;
  if (!Number.isInteger(limit) || limit <= 0)
    throw new Error(`Invalid San Diego direct breakdown limit: ${limit}`);
  const violations: SanDiegoCityDirectViolation[] = [];
  ...
  const maps = new Map<
    SanDiegoCityDirectBreakdown["categoryType"],
    Map<string, { name: string; cents: number; count: number }>
  >();
  const add = (
    type: SanDiegoCityDirectBreakdown["categoryType"],
    rawName: string | null,
    cents: number,
  ): void => {
  ...
  const breakdowns: SanDiegoCityDirectBreakdown[] = [];
