// Ballot-lookup finance loader for San Diego city (plan Phase 3). Copy of
// the San José loader (itself modeled on SF) — a bespoke loader over the
// ballotLookupFinanceShared primitives, NOT
// standardStateFinanceBallotLookupLoader: no outside-group industry
// breakdowns, per-candidate coverage notes as summary columns, real
// expenditure counts on outside groups. San Diego additionally selects and
// emits outside_coverage_note (always set by sync — the e-filed-union basis
// plus any per-candidate veto disclosure).
import type { Pool, PoolClient } from "pg";
import { isSanDiegoCampaignFinanceEnabled } from "../../config/featureFlags.js";
import {
...
import { isSanDiegoCityFinanceEligibleElection } from "./sanDiegoCityFinanceEligibleOffices.js";
...
export async function loadSanDiegoCityCandidateFinanceSummariesByCandidateElection(
  db: Queryable,
  candidateRows: readonly Candidate[],
  electionRows: readonly Election[],
): Promise<Map<string, BallotLookupFinanceSummary>> {
  if (!isSanDiegoCampaignFinanceEnabled()) return new Map();
  ...
  const summaries = await db.query<SummaryRow>(
    `
      ...
      FROM requested
      JOIN public.sdcity_candidate_finance_links link
        ON link.candidate_id=requested.candidate_id
        AND link.election_id=requested.election_id
        AND link.link_status='active'
      JOIN public.sdcity_candidate_finance_summaries summary
  ...
  const direct = await db.query<{
    ...
    `
      WITH selected AS (
        ...
        FROM selected
        JOIN public.sdcity_candidate_finance_links l
          ON l.candidate_id=selected.candidate_id
          AND l.election_id=selected.election_id
          AND l.link_status='active'
        JOIN public.sdcity_candidate_finance_direct_breakdowns b
          ON b.link_id=l.id
    ...
    `
      WITH selected AS (
        ...
        JOIN public.sdcity_candidate_finance_links l
          ...
          AND l.election_id=selected.election_id
          AND l.link_status='active'
        JOIN public.sdcity_candidate_finance_outside_groups g
          ON g.link_id=l.id
  ...
  for (const row of outside.rows) {
    ...
    list.push({
      // spender_filer_id may be the literal "Pending" — kept verbatim; the
      // (id, name) pair is the row's stable key (migration 235).
    ...
    summaries.rows.map((row) => {
      ...
      return [
        key,
        {
          ...
          direct_campaign: {
            // Donor money only: Σ(F460 line 1 + line 4). Loans stay separate
            // per the shared read contract.
            ...
            // Omitted entirely (not null) when this candidate has no known
            // gap, so complete-coverage payloads stay byte-identical.
          ...
          outside_spending: {
            ...
            // Same omit-when-null shape as the direct note; sync always sets
            // it for San Diego, so readers see the disclosure basis.
