// Phase 3 batch sync (copy of the SJ Phase 5 module), the SF due-query
// pattern reshaped for a bulk-file
...
} from "./sanDiegoCityCandidateFinanceAutoLink.js";
import {
  loadSanDiegoCityCycleWorkbookData,
  syncSanDiegoCityCandidateFinance,
  type SanDiegoCityCycleWorkbookData,
} from "./sanDiegoCityCandidateFinanceSync.js";
import { parseSanDiegoCityCouncilSeatNumber } from "./sanDiegoCityFinanceEligibleOffices.js";

type Queryable = Pick<Pool | PoolClient, "query">;
type PoolLike = Queryable & { connect: () => Promise<PoolClient> };

const SAN_DIEGO_CITY_DISTRICT_PREDICATE = `district.state='CA' AND district.district_type='place' AND district.geoid_compact='0666000'`;
...
export type SanDiegoCityCandidateFinanceBatchSyncResult = {
  dryRun: boolean;
  ...
  workbookSources: SanDiegoCityCycleWorkbookData["sources"];
  results: Array<{
    candidateId: string;
    electionId: string;
    ok: boolean;
...
const integer = (
  ...
  if (!Number.isSafeInteger(result) || result <= 0)
    throw new Error(`Invalid San Diego finance ${label}: ${result}`);
  return result;
};
...
export async function syncDueSanDiegoCityCandidateFinance(input: {
  db: PoolLike;
  now?: Date;
  dryRun?: boolean;
  ...
  loadWorkbookData?: typeof loadSanDiegoCityCycleWorkbookData;
  syncCandidateFn?: typeof syncSanDiegoCityCandidateFinance;
}): Promise<SanDiegoCityCandidateFinanceBatchSyncResult> {
  const now = input.now ?? new Date();
  if (Number.isNaN(now.getTime()))
    throw new Error("Invalid San Diego finance batch timestamp");
  ...
  if (electionId !== undefined && !UUID_PATTERN.test(electionId))
    throw new Error(`Invalid San Diego finance electionId: ${electionId}`);
  const loadWorkbookData =
    input.loadWorkbookData ?? loadSanDiegoCityCycleWorkbookData;
  const syncCandidateFn = input.syncCandidateFn ?? syncSanDiegoCityCandidateFinance;
  ...
  const workbookCache = new Map<
    number,
    { data: SanDiegoCityCycleWorkbookData } | { error: string }
  >();
  const workbookSources: SanDiegoCityCycleWorkbookData["sources"] = [];
  const getWorkbookData = async (
    electionYear: number,
  ): Promise<{ data: SanDiegoCityCycleWorkbookData } | { error: string }> => {
  ...
  if (!input.dryRun && electionId === undefined && input.autoLinkMissingLinks !== false) {
    try {
      const candidates =
        await listSanDiegoCityCandidateElectionsMissingFinanceLinks(input.db, {
          now,
          maxCandidates: max,
      ...
      for (const [electionYear, group] of byYear) {
        ...
        if ("error" in workbook) {
          console.warn(
            "San Diego finance auto-link skipped an election year; workbook unavailable:",
            { electionYear, reason: workbook.error },
          );
          continue;
        }
        const linkResults = await autoLinkMissingSanDiegoCityCandidateFinanceLinks({
          db: input.db,
          now,
          candidates: group,
        ...
        for (const row of linkResults)
          if (row.status !== "linked" && row.status !== "no_committee")
            console.warn(
              "San Diego finance auto-link did not link candidate election:",
              row,
            );
      }
    } catch (error) {
      console.warn(
        "San Diego finance auto-link skipped; continuing existing links",
  ...
  for (const row of due.rows) {
    try {
      ...
      await syncCandidateFn({
        ...
        fppcId: row.fppc_id,
        workbook: workbook.data.workbook,
        unusableRows: workbook.data.unusableRows,
