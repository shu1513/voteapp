...
import type { TexasOutsideSpendingGroup, TexasSupportOppose } from "./texasOutsideSpendingAggregator.js";
import type { TexasTecContributionRow } from "./texasTecCsvDatabaseReader.js";

export type TexasFinanceOutsideGroupBreakdown = {
  committeeId: string;
...
export type TexasOutsideGroupContributionAggregationInput = {
  electionYear: number;
...
function normalizeMinAmount(value: number | undefined): number {
  ...
  if (!Number.isFinite(value) || value < 0) {
    throw new Error(`Invalid Texas outside group contribution minIndustryAmount: ${value}`);
  }
...
function isOutsideCommitteeFiler(row: TexasTecContributionRow): boolean {
  const filerType = normalizeTextKey(row.filerTypeCd);
  return filerType === "SPAC" || filerType === "PAC" || filerType === "GPAC";
}

function donorDisplayName(row: TexasTecContributionRow): string {
  return row.contributorNameOrganization.trim();
}

function isOrganizationContributor(row: TexasTecContributionRow): boolean {
  const displayName = donorDisplayName(row);
  if (!displayName) {
    return false;
  }
  const contributorType = normalizeTextKey(row.contributorPersentTypeCd);
  return contributorType !== "INDIVIDUAL" && contributorType !== "PERSON";
}

function isOutsideDonorReceipt(input: { row: TexasTecContributionRow; electionYear: number }): boolean {
  const amountCents = parseAmountCents(input.row.contributionAmount);
  return (
    amountCents !== null &&
    amountCents > 0 &&
    !isInfoOnly(input.row) &&
    isOutsideCommitteeFiler(input.row) &&
    isCycleYear({ rawDate: input.row.contributionDt, electionYear: input.electionYear }) &&
...
function donorKey(input: { committeeId: string; supportOppose: TexasSupportOppose; normalizedName: string }): string {
  return `${normalizeId(input.committeeId)}\u0000${input.supportOppose}\u0000${input.normalizedName}`;
}

function industryKey(input: { committeeId: string; supportOppose: TexasSupportOppose; industrySlug: string }): string {
...
export function aggregateTexasOutsideGroupContributions(
  ...
  const outsideGroupsByCommitteeId = new Map<string, TexasOutsideSpendingGroup[]>();
