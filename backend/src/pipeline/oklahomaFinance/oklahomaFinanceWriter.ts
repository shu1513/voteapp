...
export async function upsertOklahomaFinanceLink(input: {
  ...
  const result = await input.db.query<ManualProtectedLinkRow>(
    `
      ...
      DO UPDATE SET
        ...
        ${manualProtectedLinkAssignments("ok_candidate_finance_links")},
  ...
  assertLinkWriteNotBlocked("Oklahoma", result.rows[0], input.link.linkSource ?? "manual", input.link.electionYear);
