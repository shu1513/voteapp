...
export type FloridaBallotFinanceCandidateRow = {
  election_id: string;
  candidate_id: string;
};

export type FloridaBallotFinanceElectionRow = {
  election_id: string;
  state: string;
};

type FloridaFinanceSummaryRequest = {
  candidate_id: string;
  election_id: string;
};

type FloridaFinanceSummaryRow = {
  candidate_id: string;
  election_id: string;
...
type FloridaFinanceDirectBreakdownRow = {
  candidate_id: string;
  election_id: string;
...
type FloridaFinanceOutsideGroupRow = {
  candidate_id: string;
  election_id: string;
  committee_id: string;
...
type FloridaFinanceOutsideIndustryRow = {
  candidate_id: string;
  election_id: string;
  support_oppose: "support" | "oppose";
  category_name: string;
...
type FloridaFinanceOutsideDonorEvidenceRow = {
  candidate_id: string;
  election_id: string;
...
const GENERIC_FLORIDA_DOS_SOURCE_URL =
...
function buildFloridaFinanceSummaryRequests(
  candidateRows: readonly FloridaBallotFinanceCandidateRow[],
  electionRows: readonly FloridaBallotFinanceElectionRow[]
): FloridaFinanceSummaryRequest[] {
  const electionIds = new Set(
    electionRows
      .filter((row) => row.state.trim().toUpperCase() === "FL")
      .map((row) => row.election_id)
  );
  const requests = new Map<string, FloridaFinanceSummaryRequest>();
...
export async function loadFloridaCandidateFinanceSummariesByCandidateElection(
  db: Queryable,
  candidateRows: readonly FloridaBallotFinanceCandidateRow[],
  electionRows: readonly FloridaBallotFinanceElectionRow[]
  ...
  const requests = buildFloridaFinanceSummaryRequests(candidateRows, electionRows);
  if (requests.length === 0) {
    return new Map();
  }

  const summaryResult = await db.query<FloridaFinanceSummaryRow>(
    `
      WITH requested AS (
        SELECT
      ...
      FROM requested
      JOIN public.fl_candidate_finance_links AS link
        ON link.candidate_id = requested.candidate_id
       AND link.election_id = requested.election_id
       AND link.link_status = 'active'
      JOIN public.fl_candidate_finance_summaries AS summary
  ...
  const directBreakdownResult = await db.query<FloridaFinanceDirectBreakdownRow>(
    `
      WITH selected AS (
        SELECT
          candidate_id::uuid AS candidate_id,
      ...
      grouped AS (
        ...
        JOIN public.fl_candidate_finance_links AS link
         ...
         AND link.link_status = 'active'
        JOIN public.fl_candidate_finance_direct_breakdowns AS breakdown
  ...
  const outsideGroupResult = await db.query<FloridaFinanceOutsideGroupRow>(
    `
      WITH selected AS (
        SELECT
          candidate_id::uuid AS candidate_id,
      ...
      grouped AS (
        ...
        JOIN public.fl_candidate_finance_links AS link
         ...
         AND link.election_id = selected.election_id
         AND link.link_status = 'active'
        JOIN public.fl_candidate_finance_outside_groups AS outside_group
  ...
  const outsideIndustryResult = await db.query<FloridaFinanceOutsideIndustryRow>(
    `
      WITH selected AS (
        SELECT
          candidate_id::uuid AS candidate_id,
      ...
      per_group AS (
        ...
        JOIN public.fl_candidate_finance_links AS link
         ...
         AND link.election_id = selected.election_id
         AND link.link_status = 'active'
        JOIN public.fl_candidate_finance_outside_group_breakdowns AS breakdown
          ON breakdown.link_id = link.id
         AND breakdown.election_year = link.election_year
  ...
  const outsideDonorEvidenceResult = await db.query<FloridaFinanceOutsideDonorEvidenceRow>(
    `
      WITH selected AS (
        SELECT
          candidate_id::uuid AS candidate_id,
      ...
      top_industries_per_group AS (
        ...
        JOIN public.fl_candidate_finance_links AS link
         ...
         AND link.election_id = selected.election_id
         AND link.link_status = 'active'
        JOIN public.fl_candidate_finance_outside_group_breakdowns AS industry
      ...
      evidence AS (
        ...
        LEFT JOIN public.fl_candidate_finance_outside_groups AS outside_group
  ...
  for (const row of directBreakdownResult.rows) {
    ...
    addFinanceBreakdown(
      ...
      row.candidate_id,
      row.election_id,
      mapFinanceBreakdown(row, summary?.source_url ?? GENERIC_FLORIDA_DOS_SOURCE_URL)
    );
  }
  ...
  for (const row of outsideGroupResult.rows) {
    ...
    list.push({
      ...
      amount: parseFinanceAmount(row.amount) ?? 0,
      source_url: firstNonEmptySourceUrl(row.source_url, GENERIC_FLORIDA_DOS_SOURCE_URL),
    });
    map.set(key, list);
  ...
  for (const row of outsideIndustryResult.rows) {
    ...
    addFinanceBreakdown(
      ...
      row.candidate_id,
      row.election_id,
      mapFinanceBreakdown(row, GENERIC_FLORIDA_DOS_SOURCE_URL)
    );
  }
