...
export async function upsertColoradoFinanceLink(input: {
  ...
  const result = await input.db.query<ManualProtectedLinkRow>(
    `
      ...
      DO UPDATE SET
        ...
        ${manualProtectedLinkAssignments("co_candidate_finance_links")},
  ...
  assertLinkWriteNotBlocked("Colorado", result.rows[0], input.link.linkSource ?? "manual", input.link.electionYear);
