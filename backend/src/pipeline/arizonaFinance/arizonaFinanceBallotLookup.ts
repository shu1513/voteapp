...
export type ArizonaFinanceBallotLookupCandidateRow = {
  candidate_id: string;
  election_id: string;
};

export type ArizonaFinanceBallotLookupElectionRow = {
  election_id: string;
  state: string;
};

type ArizonaFinanceSummaryRequest = {
  candidate_id: string;
  election_id: string;
};

type ArizonaFinanceSummaryRow = {
  candidate_id: string;
  election_id: string;
...
type ArizonaFinanceDirectBreakdownRow = {
  candidate_id: string;
  election_id: string;
...
type ArizonaFinanceOutsideGroupRow = {
  candidate_id: string;
  election_id: string;
  committee_id: string;
...
type ArizonaFinanceOutsideIndustryRow = {
  candidate_id: string;
  election_id: string;
  support_oppose: "support" | "oppose";
  category_name: string;
...
type ArizonaFinanceOutsideDonorEvidenceRow = {
  candidate_id: string;
  election_id: string;
...
function addFinanceBreakdown(
  ...
  candidateId: string,
  electionId: string,
  breakdown: BallotLookupFinanceBreakdown
): void {
...
function buildOutsideIndustrySupportExplanation(
  industryName: string,
  supportingOrganizations: readonly BallotLookupFinanceOutsideIndustrySupportEvidence[]
): string {
  const displayName = financeIndustryDisplayName(industryName);
  if (supportingOrganizations.length === 0) {
  ...
  return `The ${displayName} category is a top outside-spending support industry because ${formatShortList(
    supportingOrganizations.map((item) => item.organization_name)
  )} contributed to ${formatShortList(
    supportingOrganizations.map((item) => item.committee_name)
  )}, which reported independent spending supporting this candidate.`;
}

function buildArizonaFinanceSummaryRequests(
  candidateRows: readonly ArizonaFinanceBallotLookupCandidateRow[],
  electionRows: readonly ArizonaFinanceBallotLookupElectionRow[]
): ArizonaFinanceSummaryRequest[] {
  const electionIds = new Set(
    electionRows
      .filter((row) => row.state.trim().toUpperCase() === "AZ")
      .map((row) => row.election_id)
  );
  const requests = new Map<string, ArizonaFinanceSummaryRequest>();
...
export async function loadArizonaCandidateFinanceSummariesByCandidateElection(
  ...
  const requests = buildArizonaFinanceSummaryRequests(candidateRows, electionRows);
  if (requests.length === 0) {
    return new Map();
  }

  const summaryResult = await db.query<ArizonaFinanceSummaryRow>(
    `
      WITH requested AS (
        SELECT
      ...
      FROM requested
      JOIN public.az_candidate_finance_links AS link
        ON link.candidate_id = requested.candidate_id
       AND link.election_id = requested.election_id
       AND link.link_status = 'active'
      JOIN public.az_candidate_finance_summaries AS summary
  ...
  const directBreakdownResult = await db.query<ArizonaFinanceDirectBreakdownRow>(
    `
      WITH selected AS (
        SELECT
          candidate_id::uuid AS candidate_id,
      ...
      grouped AS (
        ...
        JOIN public.az_candidate_finance_links AS link
         ...
         AND link.link_status = 'active'
        JOIN public.az_candidate_finance_direct_breakdowns AS breakdown
  ...
  const outsideGroupResult = await db.query<ArizonaFinanceOutsideGroupRow>(
    `
      WITH selected AS (
        SELECT
          candidate_id::uuid AS candidate_id,
      ...
      grouped AS (
        ...
        JOIN public.az_candidate_finance_links AS link
         ...
         AND link.election_id = selected.election_id
         AND link.link_status = 'active'
        JOIN public.az_candidate_finance_outside_groups AS outside_group
  ...
  const outsideIndustryResult = await db.query<ArizonaFinanceOutsideIndustryRow>(
    `
      WITH selected AS (
        SELECT
          candidate_id::uuid AS candidate_id,
      ...
      per_group AS (
        ...
        JOIN public.az_candidate_finance_links AS link
         ...
         AND link.election_id = selected.election_id
         AND link.link_status = 'active'
        JOIN public.az_candidate_finance_outside_group_breakdowns AS breakdown
          ON breakdown.link_id = link.id
         AND breakdown.election_year = link.election_year
  ...
  const outsideDonorEvidenceResult = await db.query<ArizonaFinanceOutsideDonorEvidenceRow>(
    `
      WITH selected AS (
        SELECT
          candidate_id::uuid AS candidate_id,
      ...
      top_industries_per_group AS (
        ...
        JOIN public.az_candidate_finance_links AS link
         ...
         AND link.election_id = selected.election_id
         AND link.link_status = 'active'
        JOIN public.az_candidate_finance_outside_group_breakdowns AS industry
      ...
      evidence AS (
        ...
        LEFT JOIN public.az_candidate_finance_outside_groups AS outside_group
  ...
  for (const row of directBreakdownResult.rows) {
    ...
    addFinanceBreakdown(
      ...
      row.candidate_id,
      row.election_id,
      mapFinanceBreakdown(row, summary?.source_url ?? GENERIC_ARIZONA_SPOTLIGHT_SOURCE_URL)
    );
  }
  ...
  for (const row of outsideGroupResult.rows) {
    ...
    list.push({
      ...
      amount: parseFinanceAmount(row.amount) ?? 0,
      source_url: firstNonEmptySourceUrl(row.source_url, GENERIC_ARIZONA_SPOTLIGHT_SOURCE_URL),
    });
    map.set(key, list);
  ...
  for (const row of outsideIndustryResult.rows) {
    ...
    addFinanceBreakdown(
      ...
      row.candidate_id,
      row.election_id,
      mapFinanceBreakdown(row, GENERIC_ARIZONA_SPOTLIGHT_SOURCE_URL)
    );
  }
