...
export async function loadLosAngelesCandidateFinanceSummariesByCandidateElection(
  ...
  const direct = await db.query<{
    ...
    `WITH selected AS (SELECT candidate_id::uuid candidate_id,election_id::uuid election_id FROM jsonb_to_recordset($1::jsonb)x(candidate_id text,election_id text)),ranked AS (SELECT selected.candidate_id::text candidate_id,selected.election_id::text election_id,b.category_type,b.category_name,b.amount,b.contributor_count,b.source_url,row_number() OVER(PARTITION BY selected.candidate_id,selected.election_id,b.category_type ORDER BY b.amount DESC,b.category_name) rn FROM selected JOIN public.lacity_candidate_finance_links l ON l.candidate_id=selected.candidate_id AND l.election_id=selected.election_id AND l.link_status='active' JOIN public.lacity_candidate_finance_direct_breakdowns b ON b.link_id=l.id AND b.election_year=l.election_year) SELECT candidate_id,election_id,category_type,category_name,amount,contributor_count,source_url FROM ranked WHERE rn<=20 ORDER BY candidate_id,election_id,category_type,amount DESC,category_name`,
  ...
  const outside = await db.query<{
    ...
    `WITH selected AS (SELECT candidate_id::uuid candidate_id,election_id::uuid election_id FROM jsonb_to_recordset($1::jsonb)x(candidate_id text,election_id text)),ranked AS (SELECT selected.candidate_id::text candidate_id,selected.election_id::text election_id,g.spender_id,g.spender_name,g.support_oppose,g.amount,g.expenditure_count,g.source_url,row_number() OVER(PARTITION BY selected.candidate_id,selected.election_id,g.support_oppose ORDER BY g.amount DESC,g.spender_name) rn FROM selected JOIN public.lacity_candidate_finance_links l ON l.candidate_id=selected.candidate_id AND l.election_id=selected.election_id AND l.link_status='active' JOIN public.lacity_candidate_finance_outside_groups g ON g.link_id=l.id AND g.election_year=l.election_year) SELECT candidate_id,election_id,spender_id,spender_name,support_oppose,amount,expenditure_count,source_url FROM ranked WHERE rn<=5`,
    [JSON.stringify(selected)],
  );
