...
export type VirginiaCandidateFinanceSyncJobData = {
  dryRun?: boolean;
  force?: boolean;
...
export type VirginiaCandidateFinanceSyncEnqueueOptions = {
  jobId?: string;
};

type VirginiaCandidateFinanceSyncSchedulerRuntimeConfig = {
  queueName: string;
  dailyCron: string;
  dailyTz: string;
};
...
function readSchedulerRuntimeConfig(): VirginiaCandidateFinanceSyncSchedulerRuntimeConfig {
  return {
    queueName:
      process.env.VIRGINIA_CAMPAIGN_FINANCE_SYNC_SCHEDULER_QUEUE?.trim() ||
      "virginia_candidate_finance_sync_maintenance",
    dailyCron: process.env.VIRGINIA_CAMPAIGN_FINANCE_SYNC_DAILY_CRON?.trim() || "25 9 * * *",
    dailyTz: process.env.VIRGINIA_CAMPAIGN_FINANCE_SYNC_DAILY_TZ?.trim() || "UTC",
  };
}
...
function assertPositiveInteger(value: number | undefined, label: string): void {
  ...
  if (value !== undefined && (!Number.isSafeInteger(value) || value <= 0)) {
    throw new Error(`Invalid Virginia finance sync scheduler ${label}: ${value}`);
  }
}
...
function normalizeOptionalJobId(value: string | undefined): string | undefined {
  ...
  if (trimmed.includes(":")) {
    throw new Error("Virginia finance sync scheduler jobId must not contain ':'");
  }
  return trimmed;
}
...
function assertValidJobOptions(data: VirginiaCandidateFinanceSyncJobData): void {
...
export function createVirginiaCandidateFinanceSyncSchedulerQueue(): Queue<VirginiaCandidateFinanceSyncJobData> {
  return new Queue<VirginiaCandidateFinanceSyncJobData>(getQueueName(), {
    connection: getQueueConnection(),
    defaultJobOptions: defaultJobOptions(),
  });
}

export function buildVirginiaCandidateFinanceLinkedElectionSyncJobId(now = new Date()): string {
  if (!(now instanceof Date) || Number.isNaN(now.getTime())) {
    throw new Error("Invalid Virginia finance linked-election sync job date");
  }
  return `virginia-candidate-finance-linked-election-sync-${now.toISOString().slice(0, 10)}`;
}

export async function upsertRecurringVirginiaCandidateFinanceSyncJobs(
  jobData: VirginiaCandidateFinanceSyncJobData = {}
): Promise<void> {
  assertValidJobOptions(jobData);
  if (!isVirginiaCampaignFinanceEnabled()) {
    const queue = createVirginiaCandidateFinanceSyncSchedulerQueue();
    try {
      await queue.removeJobScheduler(VIRGINIA_CANDIDATE_FINANCE_SYNC_DAILY_SCHEDULER_ID);
    } finally {
      await queue.close();
    }
    return;
  ...
  try {
    await queue.upsertJobScheduler(
      VIRGINIA_CANDIDATE_FINANCE_SYNC_DAILY_SCHEDULER_ID,
      {
        pattern: config.dailyCron,
        tz: config.dailyTz,
      },
      {
        name: VIRGINIA_CANDIDATE_FINANCE_SYNC_JOB_NAME,
        data: {
...
export async function enqueueManualVirginiaCandidateFinanceSyncJob(
  jobData: VirginiaCandidateFinanceSyncJobData = {},
  options: VirginiaCandidateFinanceSyncEnqueueOptions = {}
): Promise<string> {
  assertValidJobOptions(jobData);
  if (!isVirginiaCampaignFinanceSyncEnabled(Boolean(jobData.force))) {
    return "disabled";
  }
  ...
  try {
    const job = await queue.add(
      VIRGINIA_CANDIDATE_FINANCE_SYNC_JOB_NAME,
      {
        dryRun: Boolean(jobData.dryRun),
        force: Boolean(jobData.force),
        maxCandidates: jobData.maxCandidates,
...
export async function runVirginiaCandidateFinanceSyncJob(
  data: VirginiaCandidateFinanceSyncJobData = {}
): Promise<VirginiaCandidateFinanceSyncJobResult> {
  assertValidJobOptions(data);
  const force = Boolean(data.force);
  const dryRun = Boolean(data.dryRun);
  const triggeredBy = data.triggeredBy ?? "unknown";
  const now = new Date();
  const enabled = isVirginiaCampaignFinanceSyncEnabled(force);

  if (!data.triggeredBy) {
    console.warn("Virginia finance sync job missing triggeredBy; recording as unknown");
  }
...
export function createVirginiaCandidateFinanceSyncSchedulerWorker(): Worker<
...
> {
  ...
  const processor: Processor<
    ...
    VirginiaCandidateFinanceSyncJobResult
  > = async (job) => {
    return runVirginiaCandidateFinanceSyncJob(job.data ?? {});
  };

  return new Worker<VirginiaCandidateFinanceSyncJobData, VirginiaCandidateFinanceSyncJobResult>(
    queueName,
    processor,
    {
      connection,
