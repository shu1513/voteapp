...
- `annot.py` — renders a bill PDF to text with every ADDED word wrapped in `<<...>>`.
  Indiana prints additions in bold, existing statute text in roman, and deletions in roman
  with a struck rule; `pdftotext` flattens all three. Use this instead. See
