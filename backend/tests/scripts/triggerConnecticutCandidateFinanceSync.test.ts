...
describe("triggerConnecticutCandidateFinanceSync script", () => {
  ...
  it("rejects a value flag given more than once instead of taking the first", () => {
    expect(() => parseConnecticutCandidateFinanceSyncTriggerArgs(["--max-candidates=10", "--max-candidates", "20"])).toThrow(
      "Provide --max-candidates at most once"
    );
  });

  it("rejects digit-only values above Number.MAX_SAFE_INTEGER instead of rounding them", () => {
    expect(() => parseConnecticutCandidateFinanceSyncTriggerArgs(["--max-candidates=9007199254740993"])).toThrow(
