...
import {
  ...
  EXPECTED_INCORPORATED_PLACE_ROWS_50_PLUS_DC_2024,
  parseCountyDistrictRows,
  parseIncorporatedPlaceDistrictRows,
...
import { INCORPORATED_PLACE_GEOIDS_50_PLUS_DC_2024 } from "../../../src/constants/incorporatedPlaceGeoids2024.js";
...
describe("parseIncorporatedPlaceDistrictRows", () => {
  function buildCanonicalIncorporatedPlaceData(): unknown[] {
    const data: unknown[] = [["NAME", "B01001_001E", "state", "place"]];
    for (const geoid of INCORPORATED_PLACE_GEOIDS_50_PLUS_DC_2024) {
      const state = geoid.slice(0, 2);
  ...
  it("parses complete incorporated place rows and excludes territories", () => {
    const data = buildCanonicalIncorporatedPlaceData();
    data.push(["Aceitunas comunidad, Puerto Rico", "1369", "72", "00186"]);

    const rows = parseIncorporatedPlaceDistrictRows(data);
    expect(rows).toHaveLength(EXPECTED_INCORPORATED_PLACE_ROWS_50_PLUS_DC_2024);
    expect(rows.some((row) => row.state_fips === "72")).toBe(false);
  ...
  it("allows zero population values from official place payloads", () => {
    ...
    const parsed = parseIncorporatedPlaceDistrictRows([data[0], ...rows]);
  ...
  it("throws on unexpected place code", () => {
    ...
    expect(() => parseIncorporatedPlaceDistrictRows(data)).toThrow(/Unexpected place code/);
  });

  it("throws when incorporated place rows are missing a supported state", () => {
    const allFips = Object.keys(STATE_ABBR_BY_FIPS);
    const data: unknown[] = [["NAME", "B01001_001E", "state", "place"]];
    for (const fips of allFips) {
      if (fips === "56") {
    ...
    expect(() => parseIncorporatedPlaceDistrictRows(data)).toThrow(/Missing: 56/);
  });

  it("throws when incorporated place payload is truncated even if all states are present", () => {
    const complete = buildCanonicalIncorporatedPlaceData();
    const header = complete[0];
    const rows = complete.slice(1);
    const truncated = [header, ...rows.slice(1)];
    expect(() => parseIncorporatedPlaceDistrictRows(truncated)).toThrow(
      new RegExp(`Expected ${EXPECTED_INCORPORATED_PLACE_ROWS_50_PLUS_DC_2024} incorporated place rows`)
    );
  });

  it("throws on duplicate incorporated place geoid rows", () => {
    const allFips = Object.keys(STATE_ABBR_BY_FIPS);
    const data: unknown[] = [["NAME", "B01001_001E", "state", "place"]];
    for (const fips of allFips) {
      data.push([`Place 00001, State ${fips}`, "1000", fips, "00001"]);
    ...
    expect(() => parseIncorporatedPlaceDistrictRows(data)).toThrow(/Duplicate incorporated place rows returned by Census: 0600001/);
  });

  it("throws when incorporated place payload swaps one canonical geoid for an unexpected one", () => {
    const complete = buildCanonicalIncorporatedPlaceData();
    const rows = complete.slice(1) as string[][];
    ...
    const mutated: unknown[] = [complete[0], ...rows];
    expect(() => parseIncorporatedPlaceDistrictRows(mutated)).toThrow(/Incorporated place GEOID set mismatch/);
    expect(() => parseIncorporatedPlaceDistrictRows(mutated)).toThrow(/missing=1/);
    expect(() => parseIncorporatedPlaceDistrictRows(mutated)).toThrow(/unexpected=1/);
  });
});
