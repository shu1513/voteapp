...
import { syncDueSanDiegoCityCandidateFinance } from "../../../src/pipeline/sanDiegoCityFinance/sanDiegoCityCandidateFinanceBatchSync.js";
...
function makeDb(input: {
  ...
  const query = vi.fn().mockImplementation((sql: unknown) => {
    ...
    if (s.includes("NOT EXISTS (SELECT 1 FROM public.sdcity_candidate_finance_links"))
...
describe("syncDueSanDiegoCityCandidateFinance", () => {
  ...
  it("election targeting skips auto-link and swaps the due scope", async () => {
    ...
    const syncCandidateFn = vi.fn().mockResolvedValue({});
    await syncDueSanDiegoCityCandidateFinance({
      db: db as never,
      now: NOW,
  ...
  it("rejects a malformed electionId before any work", async () => {
    ...
    ).rejects.toThrow(/Invalid San Diego finance electionId/);
