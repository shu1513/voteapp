...
describe("explainVotePower", () => {
  ...
  it("explains a fully-known contested office race with graded parts, stats, and no caveat", () => {
    ...
    expect(explanation.result).toBe("High representation + high decisiveness → Very high vote impact.");
  ...
  it("drops the margin year from the stat when it is not provided", () => {
    ...
    expect(explanation.result).toBe("Average representation + low decisiveness → Below average vote impact.");
  });
  ...
  it('displays a medium rating as "Average" in the result line', () => {
    ...
    expect(explanation.result).toBe("Average representation + average decisiveness → Average vote impact.");
  });
  ...
  it("explains an uncontested race without a margin stat", () => {
    ...
    expect(explanation.result).toBe("High representation + an uncontested race → Below average vote impact.");
  });
  ...
  it("adds a ballot-measure boost part and skips the decisiveness row when history is structurally absent", () => {
    ...
    expect(explanation.result).toBe("High representation + a ballot-measure boost → Very high vote impact.");
    expect(explanation.caveat).toBeNull();
  });
  ...
  it("keeps the decisiveness row for ballot measures that have history", () => {
    ...
    expect(explanation.result).toBe(
      "Average representation + average decisiveness + a ballot-measure boost → High vote impact."
    );
  });
  ...
  it("does not claim a boost when the missing-data cap ate the bump", () => {
    ...
    expect(explanation.result).toBe("Unknown representation + high decisiveness → High vote impact.");
  });
  ...
  it("carries a partial-data caveat when one core axis is missing", () => {
    ...
    expect(explanation.result).toBe("High representation + unknown decisiveness → High vote impact.");
