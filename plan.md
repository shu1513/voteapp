...
Remaining loader cohorts: vermont swap next (all options now exist and its pin is already green), districtOfColumbia (`committee_key` in evidence — verify at migration), newYork (industry routing), tennessee (extra `expenditure_count` + trimmed summary), oregon/pennsylvania (sum-vs-max outside totals), michigan (`candidate_loan_total`), newJersey (`::text` casts), then the family-subset states (minnesota / 4-query / 2-query / utah) if they earn their config; kentucky (dedup CTE), cities, florida (no file), and now louisiana (read transaction) stay out or get formally dropped.
