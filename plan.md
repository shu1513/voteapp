...
Next descriptor wave: wisconsin (sponsor + donor-only evidence; its summary SQL comment is text-only), alaska (`candidate_filer_id` link + `outside_group_id`/`outside_group_name` + donor-only — fully descriptor-expressible), massachusetts (`candidate_cpf_id` link + `iepac_cpf_id`/`iepac_name` + donor-only). Louisiana/vermont/newYork additionally change the direct-breakdown `category_type` filter (needs its own factory option); tennessee (extra `expenditure_count` + trimmed summary), oregon/pennsylvania (sum-vs-max outside totals) stay blocked on their variants. After that: family-subset flags (minnesota / 4-query / 2-query states) if they earn their config.
