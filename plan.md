...
Remaining loader cohorts: louisiana/vermont/newYork (direct-breakdown `category_type` filter — needs its own factory option), districtOfColumbia (`committee_key` in evidence — verify at migration), tennessee (extra `expenditure_count` + trimmed summary), oregon/pennsylvania (sum-vs-max outside totals), michigan (`candidate_loan_total`), newJersey (`::text` casts), then the family-subset states (minnesota / 4-query / 2-query / utah) if they earn their config; kentucky (dedup CTE), cities, and florida (no file) stay out or get formally dropped.
